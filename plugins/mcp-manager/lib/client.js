        ...
        factory: function (require) {
            ...
            function McpManagerSection(props) {
                ...
                var marketBody = h('div', { className: 'mm-list', 'data-testid': 'market-list' }, marketLoading && !marketLoaded ? h('p', { className: 'mm-loading', role: 'status' }, '正在读取 MCP 市场…') : visibleMarket.length ? visibleMarket.map(function (item) {
                    return h('button', { key: item.id, type: 'button', className: 'mm-row mm-rowClick mm-marketRow' + (selectedMarket && selectedMarket.id === item.id ? ' mm-rowSelected mm-marketRowSelected' : ''), onClick: function () { openMarket(item); } }, h('span', { className: 'mm-marketMain' }, h(RemoteIcon, { src: item.iconUrl }), h('span', { className: 'mm-marketCopy' }, h('span', { className: 'mm-marketTitle' }, item.repository), h('span', { className: 'mm-marketDesc' }, item.description))), h('span', { className: 'mm-rowSide mm-marketSide' }, item.status === 'installed' ? h('span', { className: 'mm-installed' }, '已安装') : null, h(P.IconChevronRightOutline14)));
                }) : h('p', { className: 'mm-empty' }, '没有匹配的 MCP 仓库。'));
                return h('section', { className: 'mm-root' + (selectedServer || selectedMarket ? ' mm-rootHasDrawer' : ''), 'aria-label': 'MCP Manager' }, h('header', { className: 'mm-head' }, h('h2', null, 'MCP'), h('span', { className: 'mm-context' }, 'Web 配置')), h('div', { className: 'mm-tabs', role: 'tablist' }, h('button', { type: 'button', role: 'tab', className: 'mm-tab' + (tab === 'servers' ? ' mm-tabOn' : ''), 'aria-selected': tab === 'servers', onClick: function () { setTab('servers'); setQuery(''); setSelectedMarket(null); } }, '服务器'), h('button', { type: 'button', role: 'tab', className: 'mm-tab' + (tab === 'market' ? ' mm-tabOn' : ''), 'aria-selected': tab === 'market', onClick: function () { setTab('market'); setQuery(''); setSelectedServer(null); } }, '市场')), tab === 'servers' ? h(React.Fragment, null, h('div', { className: 'mm-toolbar' }, h(Search, { value: query, onChange: setQuery, placeholder: '搜索服务器或工具' }), h(Button, { type: 'button', disabled: loading || refreshing, onClick: refreshServers }, h(P.IconRefreshOutline16), refreshing ? '检测中…' : '重新检测'), h(Button, { type: 'button', primary: true, onClick: function () { setEditor('new'); } }, h(P.IconPlusOutline16), '新增服务器')), h('p', { className: 'mm-helper mm-summary' }, '当前配置 web · 已连接 ' + connected + ' / ' + servers.length), h('div', { className: 'mm-filters' }, filters.map(function (item) { return h('button', { key: item[0], type: 'button', className: 'mm-filter' + (filter === item[0] ? ' mm-filterOn' : ''), onClick: function () { setFilter(item[0]); } }, item[1]); })), serverBody) : h(React.Fragment, null, h('div', { className: 'mm-toolbar' }, h(Search, { value: query, onChange: setQuery, placeholder: '搜索 GitHub 仓库' })), h('p', { className: 'mm-helper' }, '精选自 GitHub · 安装前校验 DSH MCP 安装声明'), marketBody), selectedServer ? h(ServerDrawer, { server: selectedServer, busy: busy === selectedServer.id, onClose: function () { setSelectedServer(null); }, onReconnect: function () { if (selectedServer)
