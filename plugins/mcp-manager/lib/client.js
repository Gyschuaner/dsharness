    ...
    window.__ModuleLoader__.load({
        ...
        factory: function (require) {
            ...
            function MarketDrawer(props) {
                ...
                return h('aside', { className: 'mm-drawer', role: 'dialog', 'aria-modal': false, 'aria-label': item.name + ' 详情' }, h('header', { className: 'mm-drawerHead' }, h('div', { className: 'mm-drawerTitleRow' }, h(RemoteIcon, { src: (detail && detail.iconUrl) || item.iconUrl }), h('div', { className: 'mm-drawerIdentity' }, h('h3', { className: 'mm-drawerTitle' }, item.name), h('a', { className: 'mm-drawerLink', href: sourceUrl, target: '_blank', rel: 'noreferrer' }, item.repository || item.registryName || sourceLabel, h(P.IconRightUpOutline14))), h('button', { type: 'button', className: 'mm-close', 'aria-label': '关闭详情', onClick: props.onClose }, h(P.IconCloseOutline16))), h('p', { className: 'mm-drawerDesc' }, (detail && detail.description) || item.description)), h('div', { className: 'mm-drawerBody' }, props.loading ? h('p', { className: 'mm-loading', role: 'status' }, '正在读取 GitHub 与 MCP Registry…') : null, detail && detail.metadataError ? h('p', { className: 'mm-metaNotice' }, detail.stale ? '远程元数据暂不可用，正在显示缓存。' : '部分远程元数据暂不可用。') : null, h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, '发现与安装'), h('p', { className: 'mm-metaNotice' }, '实时搜索官方 MCP Registry；仅当配置可以唯一、安全推导时允许安装，安装后默认停用。')), rows.length ? h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, '仓库信息'), h('dl', { className: 'mm-kv' }, rows.flatMap(function (row, index) { return [h('dt', { key: 'dt' + index }, row[0]), h('dd', { key: 'dd' + index }, row[1])]; }))) : null, detail && detail.topics && detail.topics.length ? h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, 'Topics'), h('div', { className: 'mm-topics' }, detail.topics.map(function (topic) { return h('span', { key: topic, className: 'mm-topic' }, topic); }))) : null, detail && detail.latestVersion ? h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, '最新发布'), h('dl', { className: 'mm-kv' }, h('dt', null, '版本'), h('dd', null, detail.latestVersion), detail.releasePublishedAt ? h(React.Fragment, null, h('dt', null, '发布时间'), h('dd', null, formatDate(detail.releasePublishedAt))) : null), detail.releaseUrl ? h('a', { className: 'mm-drawerLink', href: detail.releaseUrl, target: '_blank', rel: 'noreferrer' }, '查看 Release', h(P.IconRightUpOutline14)) : null) : null, !item.installable ? h('p', { className: 'mm-metaNotice' }, (detail && detail.installReason) || item.installReason || '当前条目无法安全推导安装配置。') : h('p', { className: 'mm-metaNotice' }, '安装会写入停用配置，不会立即执行第三方 Server；检查环境变量后再启用。')), h('footer', { className: 'mm-drawerFoot' }, h(Button, { type: 'button', onClick: function () { window.open(sourceUrl, '_blank', 'noopener,noreferrer'); } }, '查看' + sourceLabel, h(P.IconRightUpOutline14)), h(Button, { type: 'button', primary: true, disabled: props.busy || !item.installable || item.status === 'installed', onClick: props.onInstall }, props.busy ? '安装中…' : item.status === 'installed' ? '已安装' : item.installable ? '安装为停用配置' : '仅查看')));
            }
            ...
            function McpManagerSection(props) {
                ...
                function loadMarket(force, search = query, cursor = '', append = false, sort = marketSort) {
                    ...
                    return api.call('marketplace', { force: force === true, query: search.trim(), cursor: cursor, limit: 24, sort: sort }).then(function (value) {
                        ...
                        if (!append)
                            setSelectedMarket(function (current) { return current ? incoming.find(function (item) { return item.id === current.id; }) || null : null; });
                    }, function (reason) { if (request === marketRequest.current)
                        notify(reason instanceof Error ? reason.message : String(reason), true); }).finally(function () { if (request === marketRequest.current)
                ...
                if (loading)
                    ...
                    serverBody = h('div', { className: 'mm-list', 'data-testid': 'server-list' }, visibleServers.length ? visibleServers.map(function (server) {
                        return h('div', { key: server.id, className: 'mm-row mm-rowClick mm-serverRow' + (selectedServer && selectedServer.id === server.id ? ' mm-rowSelected mm-serverRowSelected' : '') }, h('button', { type: 'button', className: 'mm-serverOpen', onClick: function () { setSelectedMarket(null); setSelectedServer(server); } }, h(ServerGlyph, { transport: server.transport }), h('span', { className: 'mm-rowCopy' }, h('span', { className: 'mm-rowTitle' }, server.serverName), h('span', { className: 'mm-rowDesc' }, server.description || server.endpoint), h('span', { className: 'mm-rowMeta' }, h(Status, { status: server.status }), h('span', { className: 'mm-transport' }, server.transport === 'streamable-http' ? 'HTTP' : 'stdio'), h('span', { className: 'mm-toolCount' }, server.toolCount + ' 个工具')))), h('div', { className: 'mm-rowSide' }, h(Switch, { checked: server.enabled, disabled: busy === server.id, label: (server.enabled ? '停用 ' : '启用 ') + server.serverName, onChange: function (enabled) { toggle(server, enabled); } })));
                ...
                var marketBody = h(React.Fragment, null, marketWarning ? h('p', { className: 'mm-marketWarning', role: 'status' }, '官方 Registry 暂时不可用：' + marketWarning) : null, h('div', { className: 'mm-list', 'data-testid': 'market-list' }, marketLoading && !marketLoaded ? h('p', { className: 'mm-loading', role: 'status' }, '正在读取 MCP 官方 Registry…') : visibleMarket.length ? visibleMarket.map(function (item) {
                    return h('button', { key: item.id, type: 'button', className: 'mm-row mm-rowClick mm-marketRow' + (selectedMarket && selectedMarket.id === item.id ? ' mm-rowSelected mm-marketRowSelected' : ''), onClick: function () { openMarket(item); } }, h('span', { className: 'mm-marketMain' }, h(RemoteIcon, { src: item.iconUrl }), h('span', { className: 'mm-marketCopy' }, h('span', { className: 'mm-marketTitle' }, item.name), h('span', { className: 'mm-marketDesc' }, item.description), h('span', { className: 'mm-marketMeta' }, item.source === 'mcp-registry' ? 'MCP Registry' + (item.version ? ' · ' + item.version : '') : item.repository || '精选'))), h('span', { className: 'mm-rowSide mm-marketSide' }, item.status === 'installed' ? h('span', { className: 'mm-installed' }, '已安装') : !item.installable ? h('span', { className: 'mm-installed' }, '仅查看') : null, h(P.IconChevronRightOutline14)));
                }) : h('p', { className: 'mm-empty' }, '官方 Registry 中没有匹配的 MCP Server。')), marketNextCursor ? h('div', { className: 'mm-loadMore' }, h(Button, { type: 'button', disabled: marketLoading, onClick: function () { if (marketNextCursor)
                        loadMarket(false, query, marketNextCursor, true); } }, marketLoading ? '加载中…' : '加载更多')) : null);
                return h('section', { className: 'mm-root' + (selectedServer || selectedMarket ? ' mm-rootHasDrawer' : ''), 'aria-label': 'MCP Manager' }, h('header', { className: 'mm-head' }, h('h2', null, 'MCP')), h('div', { className: 'mm-tabs', role: 'tablist' }, h('button', { type: 'button', role: 'tab', className: 'mm-tab' + (tab === 'servers' ? ' mm-tabOn' : ''), 'aria-selected': tab === 'servers', onClick: function () { setTab('servers'); setQuery(''); setSelectedMarket(null); } }, '服务器'), h('button', { type: 'button', role: 'tab', className: 'mm-tab' + (tab === 'market' ? ' mm-tabOn' : ''), 'aria-selected': tab === 'market', onClick: function () { setTab('market'); setQuery(''); setSelectedServer(null); } }, '市场')), tab === 'servers' ? h(React.Fragment, null, h('div', { className: 'mm-toolbar' }, h(Search, { value: query, onChange: setQuery, placeholder: '搜索服务器或工具' }), h(Button, { type: 'button', disabled: loading || refreshing, onClick: refreshServers }, h(P.IconRefreshOutline16), refreshing ? '检测中…' : '重新检测'), h(Button, { type: 'button', primary: true, onClick: function () { setEditor('new'); } }, h(P.IconPlusOutline16), '新增服务器')), h('div', { className: 'mm-filters' }, filters.map(function (item) { return h('button', { key: item[0], type: 'button', className: 'mm-filter' + (filter === item[0] ? ' mm-filterOn' : ''), onClick: function () { setFilter(item[0]); } }, item[1]); })), serverBody) : h(React.Fragment, null, h('div', { className: 'mm-toolbar' }, h(Search, { value: query, onChange: setQuery, placeholder: '搜索 MCP Server' }), h('select', { className: 'mm-sort', 'aria-label': 'MCP 市场排序', value: marketSort, onChange: function (event) { setMarketSort(event.target.value); } }, h('option', { value: 'relevance' }, '综合排序'), h('option', { value: 'popular' }, '热度优先'), h('option', { value: 'recent' }, '最新优先'))), marketBody), selectedServer ? h(ServerDrawer, { server: selectedServer, busy: busy === selectedServer.id, onClose: function () { setSelectedServer(null); }, onReconnect: function () { if (selectedServer)
