	...
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			function ServerDialog(props) {
				...
				function set(name, value) { setDraft(function (current) { return Object.assign({}, current, (function () { var next = {}; next[name] = value; return next; })()); }); }
				...
				return h('div', { className: 'mm-dialogBackdrop', role: 'presentation', onMouseDown: function (event) { if (event.target === event.currentTarget && !props.busy) props.onClose(); } },
					h('form', { className: 'mm-dialog', role: 'dialog', 'aria-modal': true, 'aria-label': props.server ? '编辑 MCP 服务器' : '新增 MCP 服务器', onSubmit: submit },
						...
						h('div', { className: 'mm-formGrid' },
							h(Field, { id: 'mm-name', label: '服务器名称' }, h('input', { id: 'mm-name', className: 'mm-input', value: draft.serverName, maxLength: 32, required: true, disabled: Boolean(props.server), onChange: function (event) { set('serverName', event.target.value); } })),
							h(Field, { id: 'mm-transport', label: '传输方式' }, h('select', { id: 'mm-transport', className: 'mm-select', value: draft.transport, onChange: function (event) { set('transport', event.target.value); } }, h('option', { value: 'streamable-http' }, 'Streamable HTTP'), h('option', { value: 'stdio' }, 'stdio'))),
							h(Field, { id: 'mm-description', label: '描述', wide: true }, h('input', { id: 'mm-description', className: 'mm-input', value: draft.description, maxLength: 240, onChange: function (event) { set('description', event.target.value); } })),
							draft.transport === 'stdio' ? h(React.Fragment, null,
								h(Field, { id: 'mm-command', label: '启动命令' }, h('input', { id: 'mm-command', className: 'mm-input', value: draft.command, required: true, onChange: function (event) { set('command', event.target.value); } })),
								h(Field, { id: 'mm-cwd', label: '工作目录' }, h('input', { id: 'mm-cwd', className: 'mm-input', value: draft.cwd, placeholder: '可选绝对路径', onChange: function (event) { set('cwd', event.target.value); } })),
								h(Field, { id: 'mm-args', label: '启动参数', wide: true, help: '每行一个参数，Host 直接传递参数数组，不经过 shell 拼接。' }, h('textarea', { id: 'mm-args', className: 'mm-textarea', value: draft.argsText, onChange: function (event) { set('argsText', event.target.value); } })),
								h(Field, { id: 'mm-env', label: '环境变量引用', wide: true, help: '每行 TARGET=SOURCE_ENV；这里只保存环境变量名，不保存值。' }, h('textarea', { id: 'mm-env', className: 'mm-textarea', value: draft.envText, placeholder: 'API_KEY=MY_MCP_API_KEY', onChange: function (event) { set('envText', event.target.value); } }))
							) : h(React.Fragment, null,
								h(Field, { id: 'mm-url', label: 'MCP 端点', wide: true, help: '公网地址必须使用 HTTPS，本机地址可以使用 HTTP。' }, h('input', { id: 'mm-url', className: 'mm-input', type: 'url', value: draft.url, required: true, placeholder: 'https://example.com/mcp', onChange: function (event) { set('url', event.target.value); } })),
								h(Field, { id: 'mm-headers', label: '请求头环境变量引用', wide: true, help: '每行 HEADER=SOURCE_ENV；例如 Authorization=GITHUB_MCP_AUTHORIZATION。' }, h('textarea', { id: 'mm-headers', className: 'mm-textarea', value: draft.headersText, onChange: function (event) { set('headersText', event.target.value); } }))
							),
							h(Field, { id: 'mm-timeout', label: '工具超时（毫秒）' }, h('input', { id: 'mm-timeout', className: 'mm-input', type: 'number', min: 1000, max: 600000, step: 1000, value: draft.toolCallTimeoutMs, onChange: function (event) { set('toolCallTimeoutMs', event.target.value); } })),
							h('label', { className: 'mm-check' }, h('input', { type: 'checkbox', checked: draft.enabled, onChange: function (event) { set('enabled', event.target.checked); } }), '保存后启用'),
			...
			function McpManagerSection(props) {
				...
				function saveServer(server) {
					var editing = editor && editor.id;
					setBusy('save');
					return api.call(editing ? 'update' : 'create', editing ? { id: editor.id, server: server } : { server: server }).then(function () {
				...
				function openMarket(item) {
					...
					api.call('marketplace.detail', { id: item.id }).then(setMarketDetail, function (reason) { setMarketDetail({ metadataError: reason instanceof Error ? reason.message : String(reason) }); }).finally(function () { setDetailLoading(false); });
				}
				...
				return h('section', { className: 'mm-root' + (selectedServer || selectedMarket ? ' mm-rootHasDrawer' : ''), 'aria-label': 'MCP Manager' },
					...
					selectedServer ? h(ServerDrawer, { server: selectedServer, busy: busy === selectedServer.id, onClose: function () { setSelectedServer(null); }, onReconnect: function () { reconnect(selectedServer); }, onEdit: function () { setEditor(selectedServer); }, onDelete: function () { setDeleting(selectedServer); } }) : null,
					selectedMarket ? h(MarketDrawer, { item: selectedMarket, detail: marketDetail, loading: detailLoading, busy: busy === selectedMarket.id, onClose: function () { setSelectedMarket(null); }, onInstall: function () { installMarket(selectedMarket); } }) : null,
					editor ? h(ServerDialog, { server: editor.id ? editor : null, busy: busy === 'save', onClose: function () { if (busy !== 'save') setEditor(null); }, onSubmit: saveServer }) : null,
					deleting ? h(DeleteDialog, { server: deleting, busy: busy === 'delete', onClose: function () { if (busy !== 'delete') setDeleting(null); }, onConfirm: function () { remove(deleting); } }) : null,
