...
(function () {
    window.__ModuleLoader__.load({
        ...
        factory: function (require) {
            ...
            var existingStyle = document.querySelector('style[data-plugin="dsh-mcp-manager"]');
            ...
            style.textContent = [
                ...
                '.mm-summary{flex:none;min-height:60px;box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;display:flex;align-items:center;gap:16px;padding:10px 14px;margin-bottom:14px}',
                '.mm-summaryKey{color:var(--dsw-alias-label-tertiary);font-size:12px}',
                '.mm-summaryValue{font-size:14px;font-weight:650}',
                '.mm-summaryActions{margin-left:auto;display:flex;align-items:center;gap:10px}',
                '.mm-connected{color:var(--dsw-alias-label-secondary);white-space:nowrap}',
                ...
                '.mm-switch{appearance:none;position:relative;width:38px;height:22px;border:0;border-radius:999px;background:var(--dsw-alias-fill-tsp-secondary);cursor:pointer;padding:0;transition:background .14s ease}',
                '.mm-switch:after{content:"";position:absolute;width:18px;height:18px;left:2px;top:2px;border-radius:50%;background:var(--dsw-alias-bg-base);box-shadow:0 1px 3px rgba(0,0,0,.18);transition:transform .14s ease}',
                '.mm-switchOn{background:var(--mm-accent)}',
            ...
            function RemoteIcon(props) {
                ...
                if (!props.src || failed)
                    return h('span', { className: props.fallbackClass || 'mm-marketFallback', 'aria-hidden': true }, h(P.IconLinkOutline16, { size: props.size || 18 }));
                return h('img', {
            ...
            function statusInfo(status) {
                if (status === 'connected')
                    return { label: '已连接', className: ' mm-statusConnected' };
                if (status === 'connected-empty')
                    return { label: '已加载 · 无工具', className: ' mm-statusConnecting' };
                if (status === 'connecting')
                    return { label: '连接中', className: ' mm-statusConnecting' };
                if (status === 'failed')
                    return { label: '加载失败', className: ' mm-statusFailed' };
                if (status === 'needs-environment')
                    return { label: '缺少环境变量', className: ' mm-statusFailed' };
                if (status === 'disconnecting')
                    return { label: '断开中', className: ' mm-statusConnecting' };
                if (status === 'disabled')
                    return { label: '已停用', className: '' };
            ...
            function ServerDialog(props) {
                ...
                function set(name, value) { setDraft(function (current) { return { ...current, [name]: value }; }); }
                ...
                return h('div', { className: 'mm-dialogBackdrop', role: 'presentation', onMouseDown: function (event) { if (event.target === event.currentTarget && !props.busy)
                        props.onClose(); } }, h('form', { className: 'mm-dialog', role: 'dialog', 'aria-modal': true, 'aria-label': props.server ? '编辑 MCP 服务器' : '新增 MCP 服务器', onSubmit: submit }, h('h3', null, props.server ? '编辑服务器' : '新增服务器'), h('p', { className: 'mm-dialogLead' }, '配置保存后由 Cordis 热加载。密钥仅通过 Host 环境变量引用。'), h('div', { className: 'mm-formGrid' }, h(Field, { id: 'mm-name', label: '服务器名称' }, h('input', { id: 'mm-name', className: 'mm-input', value: draft.serverName, maxLength: 32, required: true, disabled: Boolean(props.server), onChange: function (event) { set('serverName', event.target.value); } })), h(Field, { id: 'mm-transport', label: '传输方式' }, h('select', { id: 'mm-transport', className: 'mm-select', value: draft.transport, onChange: function (event) { var value = event.target.value; if (value === 'stdio' || value === 'streamable-http')
                        set('transport', value); } }, h('option', { value: 'streamable-http' }, 'Streamable HTTP'), h('option', { value: 'stdio' }, 'stdio'))), h(Field, { id: 'mm-description', label: '描述', wide: true }, h('input', { id: 'mm-description', className: 'mm-input', value: draft.description, maxLength: 240, onChange: function (event) { set('description', event.target.value); } })), draft.transport === 'stdio' ? h(React.Fragment, null, h(Field, { id: 'mm-command', label: '启动命令' }, h('input', { id: 'mm-command', className: 'mm-input', value: draft.command, required: true, onChange: function (event) { set('command', event.target.value); } })), h(Field, { id: 'mm-cwd', label: '工作目录' }, h('input', { id: 'mm-cwd', className: 'mm-input', value: draft.cwd, placeholder: '可选绝对路径', onChange: function (event) { set('cwd', event.target.value); } })), h(Field, { id: 'mm-args', label: '启动参数', wide: true, help: '每行一个参数，Host 直接传递参数数组，不经过 shell 拼接。' }, h('textarea', { id: 'mm-args', className: 'mm-textarea', value: draft.argsText, onChange: function (event) { set('argsText', event.target.value); } })), h(Field, { id: 'mm-env', label: '环境变量引用', wide: true, help: '每行 TARGET=SOURCE_ENV；这里只保存环境变量名，不保存值。' }, h('textarea', { id: 'mm-env', className: 'mm-textarea', value: draft.envText, placeholder: 'API_KEY=MY_MCP_API_KEY', onChange: function (event) { set('envText', event.target.value); } }))) : h(React.Fragment, null, h(Field, { id: 'mm-url', label: 'MCP 端点', wide: true, help: '公网地址必须使用 HTTPS，本机地址可以使用 HTTP。' }, h('input', { id: 'mm-url', className: 'mm-input', type: 'url', value: draft.url, required: true, placeholder: 'https://example.com/mcp', onChange: function (event) { set('url', event.target.value); } })), h(Field, { id: 'mm-headers', label: '请求头环境变量引用', wide: true, help: '每行 HEADER=SOURCE_ENV；例如 Authorization=GITHUB_MCP_AUTHORIZATION。' }, h('textarea', { id: 'mm-headers', className: 'mm-textarea', value: draft.headersText, onChange: function (event) { set('headersText', event.target.value); } }))), h(Field, { id: 'mm-timeout', label: '工具超时（毫秒）' }, h('input', { id: 'mm-timeout', className: 'mm-input', type: 'number', min: 1000, max: 600000, step: 1000, value: draft.toolCallTimeoutMs, onChange: function (event) { set('toolCallTimeoutMs', event.target.value); } })), h('label', { className: 'mm-check' }, h('input', { type: 'checkbox', checked: draft.enabled, onChange: function (event) { set('enabled', event.target.checked); } }), '保存后启用'), error ? h('div', { className: 'mm-formError', role: 'alert' }, error) : null), h('div', { className: 'mm-dialogActions' }, h(Button, { type: 'button', disabled: props.busy, onClick: props.onClose }, '取消'), h(Button, { type: 'submit', primary: true, disabled: props.busy }, props.busy ? '保存中…' : '保存'))));
            }
            function DeleteDialog(props) {
                return h('div', { className: 'mm-dialogBackdrop', role: 'presentation', onMouseDown: function (event) { if (event.target === event.currentTarget && !props.busy)
                        props.onClose(); } }, h('div', { className: 'mm-dialog mm-dialogSmall', role: 'dialog', 'aria-modal': true, 'aria-label': '删除 MCP 服务器' }, h('h3', null, '删除 ' + props.server.serverName + '？'), h('p', { className: 'mm-dialogLead' }, '删除后该服务器贡献的工具会随 Cordis 热加载移除。此操作不会卸载第三方包。'), h('div', { className: 'mm-dialogActions' }, h(Button, { type: 'button', disabled: props.busy, onClick: props.onClose }, '取消'), h(Button, { type: 'button', danger: true, disabled: props.busy, onClick: props.onConfirm }, props.busy ? '删除中…' : '确认删除'))));
            }
            function ServerGlyph(props) {
            ...
            function ServerDrawer(props) {
                var server = props.server;
                var missing = server.missingEnvironment || [];
                return h('aside', { className: 'mm-drawer', role: 'dialog', 'aria-modal': false, 'aria-label': server.serverName + ' 详情' }, h('header', { className: 'mm-drawerHead' }, h('div', { className: 'mm-drawerTitleRow' }, h(ServerGlyph, { transport: server.transport }), h('div', { className: 'mm-drawerIdentity' }, h('h3', { className: 'mm-drawerTitle' }, server.serverName), h('div', { style: { marginTop: '8px' } }, h(Status, { status: server.status }))), h('button', { type: 'button', className: 'mm-close', 'aria-label': '关闭详情', onClick: props.onClose }, h(P.IconCloseOutline16))), server.description ? h('p', { className: 'mm-drawerDesc' }, server.description) : null), h('div', { className: 'mm-drawerBody' }, missing.length ? h('p', { className: 'mm-metaNotice' }, '启用前需要在 Host 环境中提供：' + missing.join('、')) : null, h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, '概览'), h('dl', { className: 'mm-kv' }, h('dt', null, '传输方式'), h('dd', null, server.transport === 'streamable-http' ? 'HTTP' : 'stdio'), h('dt', null, '端点'), h('dd', null, server.endpoint), h('dt', null, '状态来源'), h('dd', null, server.fiberPhase || 'Loader 未挂载'), h('dt', null, '工具超时'), h('dd', null, Math.round(server.toolCallTimeoutMs / 1000) + ' 秒'), h('dt', null, '最近配置'), h('dd', null, formatDate(server.updatedAt) || '—')), h(Button, { type: 'button', className: 'mm-reconnect', disabled: props.busy || !server.enabled, onClick: props.onReconnect }, h(P.IconRefreshOutline16), props.busy ? '重新连接中…' : '重新连接')), h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, '工具 (' + server.toolCount + ')'), server.tools.length ? h('div', { className: 'mm-tools' }, server.tools.map(function (tool) { return h('div', { key: tool.publicName, className: 'mm-tool' }, h('span', { className: 'mm-toolIcon' }, h(P.IconApiOutline14)), h('div', null, h('div', { className: 'mm-toolName' }, tool.name), tool.description ? h('div', { className: 'mm-toolDesc' }, tool.description) : null)); })) : h('p', { className: 'mm-empty' }, '当前没有可投影的 MCP 工具。'))), h('footer', { className: 'mm-drawerFoot' }, h(Button, { type: 'button', onClick: props.onEdit }, h(P.IconEditOutline16), '编辑配置'), h(Button, { type: 'button', danger: true, onClick: props.onDelete }, h(P.IconTrashOutline16), '删除服务器')));
            }
            function MarketDrawer(props) {
                ...
                ] : []).filter(function (row) { return typeof row[1] === 'string' && row[1] !== ''; });
                return h('aside', { className: 'mm-drawer', role: 'dialog', 'aria-modal': false, 'aria-label': item.repository + ' 详情' }, h('header', { className: 'mm-drawerHead' }, h('div', { className: 'mm-drawerTitleRow' }, h(RemoteIcon, { src: (detail && detail.iconUrl) || item.iconUrl }), h('div', { className: 'mm-drawerIdentity' }, h('h3', { className: 'mm-drawerTitle' }, item.repository.split('/')[1]), h('a', { className: 'mm-drawerLink', href: (detail && detail.url) || ('https://github.com/' + item.repository), target: '_blank', rel: 'noreferrer' }, item.repository, h(P.IconRightUpOutline14))), h('button', { type: 'button', className: 'mm-close', 'aria-label': '关闭详情', onClick: props.onClose }, h(P.IconCloseOutline16))), h('p', { className: 'mm-drawerDesc' }, (detail && detail.description) || item.description)), h('div', { className: 'mm-drawerBody' }, props.loading ? h('p', { className: 'mm-loading', role: 'status' }, '正在读取 GitHub 与 MCP Registry…') : null, detail && detail.metadataError ? h('p', { className: 'mm-metaNotice' }, detail.stale ? '远程元数据暂不可用，正在显示缓存。' : '部分远程元数据暂不可用。') : null, rows.length ? h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, '仓库信息'), h('dl', { className: 'mm-kv' }, rows.flatMap(function (row, index) { return [h('dt', { key: 'dt' + index }, row[0]), h('dd', { key: 'dd' + index }, row[1])]; }))) : null, detail && detail.topics && detail.topics.length ? h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, 'Topics'), h('div', { className: 'mm-topics' }, detail.topics.map(function (topic) { return h('span', { key: topic, className: 'mm-topic' }, topic); }))) : null, detail && detail.latestVersion ? h('section', { className: 'mm-section' }, h('h4', { className: 'mm-sectionTitle' }, '最新发布'), h('dl', { className: 'mm-kv' }, h('dt', null, '版本'), h('dd', null, detail.latestVersion), detail.releasePublishedAt ? h(React.Fragment, null, h('dt', null, '发布时间'), h('dd', null, formatDate(detail.releasePublishedAt))) : null), detail.releaseUrl ? h('a', { className: 'mm-drawerLink', href: detail.releaseUrl, target: '_blank', rel: 'noreferrer' }, '查看 Release', h(P.IconRightUpOutline14)) : null) : null), h('footer', { className: 'mm-drawerFoot' }, h(Button, { type: 'button', onClick: function () { window.open((detail && detail.url) || ('https://github.com/' + item.repository), '_blank', 'noopener,noreferrer'); } }, '在 GitHub 查看', h(P.IconRightUpOutline14)), h(Button, { type: 'button', primary: true, disabled: props.busy || !item.installable || item.status === 'installed', onClick: props.onInstall }, props.busy ? '安装中…' : item.status === 'installed' ? '已安装' : item.installable ? '安装' : '选择具体服务器')));
            }
            function McpConnectingState() {
                return h('div', { className: 'mm-connectingState', role: 'status', 'aria-live': 'polite', 'aria-atomic': 'true' }, h('div', { className: 'mm-connectingVisual', 'aria-hidden': true }, h('span', { className: 'mm-connectingEndpoint mm-connectingEndpointLocal' }, h(P.IconCodeOutline16)), h('span', { className: 'mm-connectingEndpoint mm-connectingEndpointRemote' }, h(P.IconApiOutline14)), h('span', { className: 'mm-connectingCore' }, h(P.IconLinkOutline16))), h('span', { className: 'mm-connectingLabel', 'data-text': 'MCP Connecting' }, 'MCP Connecting', h('span', { className: 'mm-connectingCursor', 'aria-hidden': true })));
            }
            function McpManagerSection(props) {
                ...
                function loadServers() {
                    return api.call('list').then(function (value) {
                        ...
                        setConnected(value.connected || 0);
                ...
                React.useEffect(function () { if (tab === 'market' && !marketLoaded)
                    loadMarket(false); }, [tab, marketLoaded]);
                React.useEffect(function () {
                ...
                function saveServer(server) {
                    ...
                    return api.call(editing ? 'update' : 'create', editing ? { id: editing, server: server } : { server: server }).then(function () {
                        setEditor(null);
                        notify(editing ? '服务器配置已保存并触发热加载' : '服务器已添加并触发热加载', false);
                        return loadServers();
                ...
                var visibleServers = servers.filter(function (server) {
                    ...
                    if (!matchText || filter === 'all')
                        return matchText;
                    if (filter === 'connected')
                        return server.status === 'connected' || server.status === 'connected-empty';
                    if (filter === 'failed')
                        return server.status === 'failed' || server.status === 'needs-environment';
                ...
                var filters = [['all', '全部 ' + servers.length], ['connected', '已连接 ' + connected], ['failed', '连接问题 ' + servers.filter(function (server) { return server.status === 'failed' || server.status === 'needs-environment'; }).length], ['disabled', '已停用 ' + servers.filter(function (server) { return server.status === 'disabled'; }).length]];
                var serverBody;
                if (loading)
                    serverBody = h(McpConnectingState);
                else if (error)
                    serverBody = h('div', { className: 'mm-loadError' }, h('p', { className: 'mm-error', role: 'alert' }, '加载 MCP Manager 失败：' + error), h(Button, { type: 'button', onClick: function () { setAttempt(function (value) { return value + 1; }); } }, h(P.IconRefreshOutline16), '重试'));
                else
                    serverBody = h('div', { className: 'mm-table', 'data-testid': 'server-list' }, h('div', { className: 'mm-tableHead', role: 'row' }, h('span', null, '服务器'), h('span', null, '状态'), h('span', { className: 'mm-transportCol' }, '传输方式'), h('span', { className: 'mm-toolCol' }, '工具数'), h('span', null, '启用')), visibleServers.length ? visibleServers.map(function (server) {
                        return h('div', { key: server.id, className: 'mm-serverRow' + (selectedServer && selectedServer.id === server.id ? ' mm-serverRowSelected' : '') }, h('button', { type: 'button', className: 'mm-serverOpen', onClick: function () { setSelectedMarket(null); setSelectedServer(server); } }, h(ServerGlyph, { transport: server.transport }), h('span', { className: 'mm-serverCopy' }, h('span', { className: 'mm-serverName' }, server.serverName), h('span', { className: 'mm-serverDesc' }, server.description || server.endpoint))), h(Status, { status: server.status }), h('span', { className: 'mm-transport mm-transportCol' }, server.transport === 'streamable-http' ? 'HTTP' : 'stdio'), h('span', { className: 'mm-toolCount mm-toolCol' }, server.toolCount + ' 个工具'), h(Switch, { checked: server.enabled, disabled: busy === server.id, label: (server.enabled ? '停用 ' : '启用 ') + server.serverName, onChange: function (enabled) { toggle(server, enabled); } }));
                ...
                var marketBody = h('div', { className: 'mm-marketList', 'data-testid': 'market-list' }, marketLoading && !marketLoaded ? h('p', { className: 'mm-loading', role: 'status' }, '正在读取 MCP 市场…') : visibleMarket.length ? visibleMarket.map(function (item) {
                    return h('button', { key: item.id, type: 'button', className: 'mm-marketRow' + (selectedMarket && selectedMarket.id === item.id ? ' mm-marketRowSelected' : ''), onClick: function () { openMarket(item); } }, h('span', { className: 'mm-marketMain' }, h(RemoteIcon, { src: item.iconUrl }), h('span', { className: 'mm-marketCopy' }, h('span', { className: 'mm-marketTitle' }, item.repository), h('span', { className: 'mm-marketDesc' }, item.description))), h('span', { className: 'mm-marketSide' }, item.status === 'installed' ? h('span', { className: 'mm-installed' }, '已安装') : null, h(P.IconChevronRightOutline14)));
                }) : h('p', { className: 'mm-empty' }, '没有匹配的 MCP 仓库。'));
                return h('section', { className: 'mm-root' + (selectedServer || selectedMarket ? ' mm-rootHasDrawer' : ''), 'aria-label': 'MCP Manager' }, h('div', { className: 'mm-tabs', role: 'tablist' }, h('button', { type: 'button', role: 'tab', className: 'mm-tab' + (tab === 'servers' ? ' mm-tabOn' : ''), 'aria-selected': tab === 'servers', onClick: function () { setTab('servers'); setQuery(''); setSelectedMarket(null); } }, '服务器', h('span', { className: 'mm-count' }, servers.length)), h('button', { type: 'button', role: 'tab', className: 'mm-tab' + (tab === 'market' ? ' mm-tabOn' : ''), 'aria-selected': tab === 'market', onClick: function () { setTab('market'); setQuery(''); setSelectedServer(null); } }, '市场')), tab === 'servers' ? h('div', { className: 'mm-content' }, h('div', { className: 'mm-summary' }, h('span', { className: 'mm-summaryKey' }, '当前配置'), h('span', { className: 'mm-summaryValue' }, 'web'), h('div', { className: 'mm-summaryActions' }, h('span', { className: 'mm-connected' }, '已连接 ' + connected + ' / ' + servers.length), h(Button, { type: 'button', disabled: loading || refreshing, onClick: refreshServers }, h(P.IconRefreshOutline16), refreshing ? '检测中…' : '重新检测'), h(Button, { type: 'button', primary: true, onClick: function () { setEditor('new'); } }, h(P.IconPlusOutline16), '新增服务器'))), h(Search, { value: query, onChange: setQuery, placeholder: '搜索服务器或工具' }), h('div', { className: 'mm-filters' }, filters.map(function (item) { return h('button', { key: item[0], type: 'button', className: 'mm-filter' + (filter === item[0] ? ' mm-filterOn' : ''), onClick: function () { setFilter(item[0]); } }, item[1]); })), serverBody) : h('div', { className: 'mm-content' }, h(Search, { value: query, onChange: setQuery, placeholder: '搜索 GitHub 仓库' }), marketBody), selectedServer ? h(ServerDrawer, { server: selectedServer, busy: busy === selectedServer.id, onClose: function () { setSelectedServer(null); }, onReconnect: function () { if (selectedServer)
                        reconnect(selectedServer); }, onEdit: function () { if (selectedServer)
                        setEditor(selectedServer); }, onDelete: function () { if (selectedServer)
                        setDeleting(selectedServer); } }) : null, selectedMarket ? h(MarketDrawer, { item: selectedMarket, detail: marketDetail, loading: detailLoading, busy: busy === selectedMarket.id, onClose: function () { setSelectedMarket(null); }, onInstall: function () { if (selectedMarket)
                        installMarket(selectedMarket); } }) : null, editor ? h(ServerDialog, { server: typeof editor === 'object' ? editor : null, busy: busy === 'save', onClose: function () { if (busy !== 'save')
                        setEditor(null); }, onSubmit: saveServer }) : null, deleting ? h(DeleteDialog, { server: deleting, busy: busy === 'delete', onClose: function () { if (busy !== 'delete')
                        setDeleting(null); }, onConfirm: function () { if (deleting)
                        remove(deleting); } }) : null, toast ? h('div', { className: 'mm-toast' + (toast.error ? ' mm-toastError' : ''), role: 'status' }, toast.message) : null);
            }
            var module = { exports: {} };
