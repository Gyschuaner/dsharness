...
function environmentProjection(value) {
    const rows = Array.isArray(value) ? value : [];
    const env = {};
    const required = [];
    for (const rowValue of rows) {
        if (!isRecord(rowValue) || !ENV_NAME_RE.test(String(rowValue.name || '')))
            return null;
        const name = String(rowValue.name);
        env[name] = name;
        if (rowValue.isRequired === true)
            required.push(name);
    }
    return { env, required };
}
function argumentValues(value) {
    ...
    const result = [];
    for (const rowValue of rows) {
        if (!isRecord(rowValue) || rowValue.type !== 'positional' || typeof rowValue.value !== 'string' || rowValue.value.includes('\0'))
            return null;
        result.push(rowValue.value);
    }
    return result;
}
function inferRemoteInstall(server, serverName, description) {
...
function inferPackageInstall(server, serverName, description) {
    ...
    if (runtimeArgs === null || packageArgs === null || environment === null)
        return null;
