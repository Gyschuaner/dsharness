...
export function createMcpManager(options: McpManagerOptions = {}) {
	...
	function marketplaceSort(value: unknown): MarketplaceSort {
		...
		if (sort !== 'relevance' && sort !== 'popular' && sort !== 'recent') throw new ApiError(400, '市场排序无效', 'MARKET_SORT_INVALID');
		return sort;
	}

	function rankMarketplace(entries: MarketplaceEntry[], sort: MarketplaceSort): MarketplaceEntry[] {
		...
		return entries.map((entry, index) => ({ entry, index })).sort((left, right) => {
			...
			if (sort === 'popular') {
				const score = (entry: MarketplaceEntry) => (entry.source === 'featured' ? 1_000 : 0) + (entry.install !== null ? 100 : 0) + (entry.repository !== null ? 10 : 0);
	...
	async function officialRegistryPage(query: string, cursor: string, limit: number, force: boolean, sort: MarketplaceSort) {
