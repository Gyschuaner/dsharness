...
(function () {
    window.__ModuleLoader__.load({
        ...
        factory: function (require) {
            ...
            var existingStyle = document.querySelector('style[data-plugin="dsh-extension-manager"]');
            ...
            function readNavCollapsed() {
                try {
                    ...
                    if (value === null)
                        value = window.localStorage.getItem('smgr.ext.navCollapsed');
                    return value === '1';
                }
                catch (error) {
                    return false;
            ...
            function ExtensionsPage(props) {
                ...
                function toggleNav() {
                    ...
                    try {
                        window.localStorage.setItem('dsh.extensions.navCollapsed', next ? '1' : '0');
                    }
                    catch (error) { }
                }
                ...
                return h('div', { className: 'ext-page', role: 'region', 'aria-label': '扩展管理' }, h('header', { className: 'ext-top' }, h('div', { className: 'ext-topTitle' }, h(ExtIcon, { size: 20 }), '扩展'), h('span', { className: 'ext-topSub' }, active ? '/ ' + active.toUpperCase() : ''), h('button', { type: 'button', className: 'ext-close', 'aria-label': '关闭扩展页', title: '关闭（Esc）', onClick: onClose }, h(P.IconCloseOutline16))), h('div', { className: 'ext-body' }, h('nav', { className: 'ext-nav' + (navCollapsed ? ' ext-navCollapsed' : ''), 'aria-label': '扩展类型' }, h('div', { className: 'ext-navHead' }, h('span', { className: 'ext-navTitle' }, '扩展类型'), h('button', {
                ...
                }, h(navCollapsed ? P.IconChevronRightOutline14 : P.IconChevronLeftOutline14))), h('div', { className: 'ext-navList' }, rows.map(function (row) {
                    ...
                    }, h('span', { className: 'ext-navIcon' }, h(Icon), navCollapsed && row.soon ? h('span', { className: 'ext-navSoonDot', 'aria-hidden': true }) : null), h('span', { className: 'ext-navCopy' }, h('span', { className: 'ext-navLabel' }, row.label, row.soon ? h('span', { className: 'ext-soon' }, '建设中') : null)));
            ...
            function ExtensionsEntry(props) {
                ...
                return h(React.Fragment, null, h('div', { className: 'ext-layer' + (props.wide ? '' : ' ext-layerRail') }, h('button', {
                ...
                }, h(ExtIcon, { size: props.wide ? 16 : 18 }), props.wide ? h('span', { className: 'ext-triggerLabel' }, '扩展') : null)), open ? h(ExtensionsPage, { rows: rows, renderSlot: props.renderSlot, onClose: function () { setOpen(false); } }) : null);
            }
            var module = { exports: {} };
