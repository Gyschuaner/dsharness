...
interface BillingBackend {
	summary(request?: { from?: number; to?: number; sessionId?: string }): Promise<BillingSummary>
}
...
export function makeHandler(options: HandlerOptions = {}): RequestHandler {
	...
	return async (req, res): Promise<void> => {
		...
		try {
			if (req.method !== 'POST') throw new BillingApiError(405, 'METHOD_NOT_ALLOWED', 'method not allowed')
			const chunks: Buffer[] = []
			let bytes = 0
			for await (const chunk of req) {
				const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk)
				bytes += buffer.length
				if (bytes > 64 * 1024) throw new BillingApiError(413, 'BODY_TOO_LARGE', '请求体过大')
				chunks.push(buffer)
			}
			let body: unknown
			try {
			...
			if (!isRecord(body)) throw new BillingApiError(400, 'BODY_INVALID', '请求体必须是对象')
			...
			const request: { from?: number; to?: number; sessionId?: string } = {}
			...
			if (status >= 500) logger?.warn?.(`billing: ${error instanceof Error ? error.stack : message}`)
...
function apply(ctx: HostContext): void {
	...
	ctx.effect(() => {
		...
		return () => {
			disposeRoute()
			if (store instanceof BillingStore) store.close()
		}
	}, 'billing: web route')
}
