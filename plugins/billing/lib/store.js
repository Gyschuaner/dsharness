...
export class BillingStore {
    ...
    syncNow() {
        ...
        for (const path of listLogFiles(this.sessionRoot)) {
            ...
            const previous = this.db.prepare('SELECT size, mtime_ms FROM billing_watermarks WHERE source_path = ?').get(path);
            if (previous?.size === metadata.size && previous.mtime_ms === metadata.mtimeMs)
                continue;
            let parsed;
            try {
            ...
            try {
                ...
                for (const call of parsed.calls) {
                    insert.run(call.callKey, path, call.sessionId, call.sessionTitle, call.model, call.timestamp, call.turn, call.step, call.inputTokens, call.outputTokens, call.cacheReadTokens, call.cacheWriteTokens, call.estimatedCost, call.priceMode, call.priceReason);
                }
                this.db.prepare(`
        ...
        for (const path of this.issuesByPath.keys())
            if (!seen.has(path))
    ...
    async summary(request = {}) {
		...
		`).all(...values);
        const daily = dailyRows.map((row) => ({ date: row.date ?? '', ...aggregate(row) }));
