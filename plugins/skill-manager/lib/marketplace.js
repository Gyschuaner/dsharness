...
export function parseGitHubSkillUrl(value) {
    ...
    try {
        ...
        throw new ApiError(400, '请输入有效的 GitHub URL', 'GITHUB_URL_INVALID');
    }
    if (parsed.protocol !== 'https:' || parsed.hostname.toLowerCase() !== 'github.com')
        throw new ApiError(400, '仅支持 https://github.com 上的公开仓库', 'GITHUB_URL_UNSUPPORTED');
    const parts = parsed.pathname.split('/').filter(Boolean).map((part) => decodeURIComponent(part));
    if (parts.length < 2)
        throw new ApiError(400, 'GitHub URL 必须包含 owner/repository', 'GITHUB_URL_INVALID');
    const repository = `${parts[0]}/${String(parts[1]).replace(/\.git$/i, '')}`;
    if (!REPOSITORY_RE.test(repository))
        throw new ApiError(400, 'GitHub 仓库名称不合法', 'GITHUB_REPOSITORY_INVALID');
    let ref = 'main';
    let path = null;
    let explicitRef = false;
    if (parts.length > 2) {
        if (parts[2] !== 'tree' || parts.length < 4)
            throw new ApiError(400, '请使用仓库主页或 /tree/<ref>/<Skill目录> URL', 'GITHUB_URL_INVALID');
        ref = parts[3];
        explicitRef = true;
        path = parts.slice(4).join('/') || null;
        if (path !== null && !safeRelativePath(path))
            throw new ApiError(400, 'GitHub Skill 路径不安全', 'GITHUB_PATH_UNSAFE');
    }
    if (ref.trim() === '' || ref.includes('..'))
        throw new ApiError(400, 'GitHub ref 不合法', 'GITHUB_REF_INVALID');
...
function parseRepositoryArchive(data) {
    let tar;
    try {
        tar = gunzipSync(data, { maxOutputLength: MAX_ARCHIVE_EXPANDED_BYTES });
    }
    catch {
        throw new ApiError(502, 'GitHub 仓库归档无法解压或超过 120MB 上限', 'GITHUB_ARCHIVE_INVALID');
    }
    const entries = [];
    for (let offset = 0; offset + 512 <= tar.length;) {
        ...
        if (!Number.isFinite(size) || size < 0 || size > MAX_ARCHIVE_EXPANDED_BYTES)
            throw new ApiError(502, 'GitHub 仓库归档包含无效文件大小', 'GITHUB_ARCHIVE_INVALID');
        const type = String.fromCharCode(header[156] || 48);
        const bodyStart = offset + 512;
        const bodyEnd = bodyStart + size;
        if (bodyEnd > tar.length)
            throw new ApiError(502, 'GitHub 仓库归档已截断', 'GITHUB_ARCHIVE_INVALID');
        const slash = fullName.indexOf('/');
        const path = slash === -1 ? '' : fullName.slice(slash + 1).replace(/\/$/, '');
        if (path !== '')
            entries.push({ path, type, data: type === '0' || type === '\0' ? Buffer.from(tar.subarray(bodyStart, bodyEnd)) : null });
        if (entries.length > 20_000)
            throw new ApiError(413, 'GitHub 仓库归档条目过多', 'GITHUB_ARCHIVE_TOO_MANY_ENTRIES');
...
export function createMarketplace(options = {}) {
    ...
    function trustedIndexEntries(index, value) {
        ...
        for (const pluginValue of plugins) {
            ...
            for (const skillValue of Array.isArray(pluginValue.skills) ? pluginValue.skills : []) {
                ...
                if (!safeRelativePath(path) || !NAME_RE.test(name))
                    continue;
                try {
    ...
    async function discoverEntries(force = false) {
        ...
        for (const index of TRUSTED_SKILL_INDEXES) {
            try {
                ...
                for (const entry of trustedIndexEntries(index, result.value))
                    discoveredEntries.set(entry.id, entry);
                if (result.error)
                    warnings.push(`${index.label}：${result.error}`);
            }
            catch (error) {
    ...
    async function loadBundle(entry, enforceName = true) {
        ...
        try {
            ...
            if (selected.some((item) => item.mode === '120000'))
                throw new ApiError(409, '远程 Skill 包含符号链接，拒绝安装', 'MARKET_SYMLINK_REJECTED');
            ...
            if (selected.some((item) => item.type === '1' || item.type === '2'))
                throw new ApiError(409, '远程 Skill 包含符号链接，拒绝安装', 'MARKET_SYMLINK_REJECTED');
            output = selected
                ...
                .filter((item) => safeRelativePath(item.path))
                .sort((a, b) => a.path.localeCompare(b.path));
            if (output.length === 0 || !output.some((file) => file.path === 'SKILL.md'))
                throw new ApiError(404, '市场条目缺少 SKILL.md', 'MARKET_SKILL_NOT_FOUND');
        ...
        if (enforceName && manifest.name !== entry.name)
    ...
    async function inspectGithub(url) {
        ...
        if (!parsed.explicitRef) {
            try {
                ...
                if (isRecord(repoValue) && typeof repoValue.default_branch === 'string' && repoValue.default_branch.trim() !== '')
                    ref = repoValue.default_branch;
            }
        ...
        try {
            const treeValue = await requestJson(treePath(entry));
            if (isRecord(treeValue) && treeValue.truncated === true)
                throw new ApiError(502, 'GitHub 返回的仓库文件树被截断，暂不允许安装', 'MARKET_TREE_TRUNCATED');
            const tree = isRecord(treeValue) && Array.isArray(treeValue.tree) ? treeValue.tree : [];
            manifestPaths = tree
        ...
        if (candidates.length === 0)
            throw new ApiError(404, requested === null ? '仓库中没有找到目录型 SKILL.md' : `指定目录缺少 SKILL.md：${requested}`, 'MARKET_SKILL_NOT_FOUND');
        if (candidates.length > 100)
            throw new ApiError(413, '仓库中的 Skill 候选超过 100 个，请输入具体目录 URL', 'MARKET_TOO_MANY_CANDIDATES');
        return {
    ...
    async function githubEntry(url, requestedPath) {
        ...
        if (path === null)
            throw new ApiError(409, '仓库包含多个 Skill，请先选择要安装的目录', 'GITHUB_PATH_REQUIRED');
        if (!inspected.candidates.some((candidate) => candidate.path === path))
            throw new ApiError(400, '所选目录不在已校验的 Skill 候选中', 'GITHUB_PATH_INVALID');
        const base = {
    ...
    async function projectItemState(projectRoot, state, entry, meta) {
        ...
        if (existing.hash !== (selection.originBundleHash || selection.marketHash))
