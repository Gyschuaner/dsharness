...
(function () {
    window.__ModuleLoader__.load({
        ...
        factory: function (require) {
            ...
            var existingStyle = document.querySelector('style[data-plugin="dsh-skill-manager"]');
                ...
                return fetch('/api/skill-manager', {
                    ...
                    if (!res.ok) {
                        return res.json().then(function (data) {
                            ...
                            if (msg && msg.indexOf('exportZip') !== -1)
                                throw new Error('zip 导出需要重启 dsh web 后生效（host 端尚未加载新操作）');
            ...
            function groupSkills(skills) {
                ...
                for (let i = 0; i < skills.length; i += 1) {
                    ...
                    if (prefix === null || skill.isShadow) {
                        items.push({ kind: 'skill', skill: skill });
                        continue;
                    }
                    if (emitted[prefix])
                        continue;
                    ...
                    if (members.length >= 3)
                        items.push({ kind: 'package', prefix: prefix, skills: members });
                    else
                        for (let k = 0; k < members.length; k += 1)
                            items.push({ kind: 'skill', skill: members[k] });
                }
                return items;
            }
            ...
            function writePkgMeta(rootId, prefix, patch) {
                ...
                try {
                    window.localStorage.setItem('smgr.pkg.' + rootId + '.' + prefix, JSON.stringify(next));
                }
                catch (error) { }
                return next;
            }
            ...
            function SkillManagerSection(props) {
                ...
                const load = React.useCallback(function () {
                    setBusy(true);
                    setError(null);
                    return api.call('list', {}).then(function (value) { setData(value); setBusy(false); }, function (e) { setError(String((e && e.message) || e)); setBusy(false); });
                }, [api]);
                ...
                function openDetail(root, name) {
                    ...
                    setDetail(null);
                    api.call('read', { root: root, name: name }).then(function (value) { setDetail(value); }, function (e) { setError(String((e && e.message) || e)); setDetail(null); });
                }
                function doDelete() {
                    if (!sel)
                        return;
                    setBusy(true);
                    setError(null);
                    api.call('delete', { root: sel.root, name: sel.name }).then(function () {
                        setBusy(false);
                ...
                function doImport() {
                    setBusy(true);
                    setError(null);
                    api.call('import', { root: importRoot, content: importText }).then(function (value) {
                        setBusy(false);
                        setImportOpen(false);
                        setImportText('');
                ...
                function togglePolicy() {
                    if (!policyReady || policyBusy)
                        return;
                    setPolicyBusy(true);
                    setError(null);
                    api.call('setPolicy', { globalDefaultOff: !policyOn }).then(function () {
                        setPolicyBusy(false);
                        void load();
                ...
                function toggleSkill(root, skill) {
                    ...
                    if (!switchReady || !projectReady || toggling !== null)
                        return;
                    setToggling(key);
                    setError(null);
                    const wantDisabled = skill.disabled !== true;
                    api.call('setStatus', { root: root.id, name: skill.name, disabled: wantDisabled }).then(function () {
                        setToggling(null);
                ...
                function bulkToggle(root, skills) {
                    if (!switchReady || !projectReady || bulkBusy)
                        return;
                    ...
                    chain.then(function () {
                        setBulkBusy(false);
                        if (failed.length > 0)
                            setError('批量开关有 ' + failed.length + ' 个失败：' + failed.slice(0, 3).join('；') + (failed.length > 3 ? ' …' : ''));
                        void load();
                    });
                }
                ...
                function doExportPkgZip(rootId, skills) {
                    ...
                    setBusy(true);
                    setError(null);
                    api.zip(rootId, skills.map(function (s) { return s.name; })).then(function (filename) { setBusy(false); flash('已导出 ' + filename); }, function (e) { setBusy(false); setError(String((e && e.message) || e)); });
                }
                function doPkgDelete() {
                    ...
                    chain.then(function () {
                        ...
                        setConfirmPkg(null);
                        if (failed.length > 0)
                            setError('批量删除有 ' + failed.length + ' 个失败：' + failed.slice(0, 3).join('；') + (failed.length > 3 ? ' …' : ''));
                        else
                            flash('已删除技能包「' + target.label + '」（' + target.names.length + ' 个）');
                        void load();
                    });
                }
                // ── rendering ─────────────────────────────────────────────────
                ...
                function skillRow(skill, root) {
                    ...
                    return h('div', { key: skill.name, className: 'smgr-row' + (active ? ' smgr-rowActive' : '') + (skill.disabled === true ? ' smgr-rowOff' : ''), onClick: function () { openDetail(root.id, skill.name); } }, h('div', { className: 'smgr-rowMain' }, h('span', { className: 'smgr-name' }, skill.title || skill.name, skill.broken
                        ? h('span', { className: 'smgr-badge smgr-badgeBad', title: skill.broken }, '格式损坏')
                        : null, skill.isShadow ? h('span', { className: 'smgr-badge smgr-badgeShadow', title: '项目级禁用开关文件（由开关生成）' }, '禁用开关') : null, skill.disabled === true && !skill.isShadow ? h('span', { className: 'smgr-badge smgr-badgeOff' }, '已禁用') : null), skill.description ? h('span', { className: 'smgr-desc' }, skill.description) : null), switchEl(root, skill), h('div', {
                        ...
                        title: '导出 .md',
                        onClick: function () {
                            api.call('read', { root: root.id, name: skill.name }).then(function (value) { exportSkill(skill, value.content); }, function (e) { setError(String((e && e.message) || e)); });
                        }
                    }, '⇩')));
                }
                function packageRow(root, item, expanded, meta) {
                    ...
                    return h('div', { className: 'smgr-pkgRow' + (offCount === all.length ? ' smgr-pkgRowOff' : '') }, h('button', {
                        className: 'smgr-caret',
                        title: expanded ? '折叠' : '展开',
                        onClick: function (event) { event.stopPropagation(); togglePkg(root.id, item.prefix, expanded); }
                    }, expanded ? '▾' : '▸'), h('span', { className: 'smgr-pkgIcon', 'aria-hidden': true }), h('div', { className: 'smgr-rowMain', onClick: function () { togglePkg(root.id, item.prefix, expanded); } }, h('span', { className: 'smgr-name' }, isEditing
                        ? h('input', {
                        ...
                        : h('span', { className: 'smgr-pkgLabel' }, label), h('span', { className: 'smgr-badge' }, '技能包 ×' + all.length), offCount > 0 ? h('span', { className: 'smgr-badge smgr-badgeOff' }, offCount + ' 已禁用') : null, h('button', {
                        className: 'smgr-iconBtn',
                        title: '修改显示名（仅保存在本机浏览器）',
                        onClick: function (event) { event.stopPropagation(); startLabelEdit(root.id, item.prefix, label); }
                    }, '✎')), h('span', { className: 'smgr-desc' }, '前缀 ' + item.prefix + '- · ' + sample)), h('div', { className: 'smgr-rowActions', onClick: function (event) { event.stopPropagation(); } }, (switchReady && projectReady)
                        ? h('button', {
                    ...
                    for (const item of items) {
                        if (item.kind === 'skill') {
                            ...
                            if (skill.isShadow) {
                                ...
                                if (q === '' || !matches(skill, q))
                                    continue;
                            }
                            else {
                                if (!matches(skill, q) || !passesFilter(skill))
                                    continue;
                            }
                        ...
                        pkgOut.push(h(React.Fragment, { key: 'p:' + item.prefix }, packageRow(root, item, expanded, meta), expanded
                ...
                return h('div', { className: 'smgr' }, h('h2', null, 'Skills 技能管理'), h('p', { className: 'smgr-intro' }, '浏览、导入导出 skill。项目级只影响本项目，用户级影响所有项目，内置只读。每行的小滑块按项目启用/禁用：关掉后模型在本项目不再自动调用它（手动调用不受影响）。导入/删除后自动热加载，无需重启。'), h('div', { className: 'smgr-policy' }, h('div', { className: 'smgr-policyMain' }, h('span', { className: 'smgr-name' }, '全局默认关闭', h('span', { className: 'smgr-badge' }, policyOn ? '已开启' : '已关闭')), h('span', { className: 'smgr-policyDesc' }, '开启后用户级 skill 默认禁用（内置不受影响）；某项目要用某个 skill，打开它的滑块即可（生成项目本地副本）。')), h('button', {
                    type: 'button',
                ...
                }, h('span', { className: 'smgr-switchKnob' }))), cwd !== null && cwd !== undefined
                    ? h('p', { className: 'smgr-cwd' }, '当前工作区：', h('code', null, cwd), projectRoot !== null && projectRoot !== undefined
                        ? h('span', null, '　开关生效的项目根：', h('code', null, projectRoot))
                        : null)
                    : h('p', { className: 'smgr-cwd' }, '当前页没有会话工作区：仅显示用户级与内置 skill，按项目开关不可用。'), h('div', { className: 'smgr-toolbar' }, h('input', {
                    className: 'smgr-search',
                    placeholder: '搜索名称或描述…',
                    value: q,
                    onChange: function (event) { setQ(event.target.value); }
                }), h('div', { className: 'smgr-filter' }, [
                    ['all', '全部'],
                ...
                })), h(Button, { variant: 'outline', disabled: busy, onClick: function () { void load(); } }, busy ? '加载中…' : '刷新'), h(Button, {
                    variant: 'outline',
                    onClick: function () {
                        setImportText('');
                ...
                }, '导入 Skill')), notice !== null ? h('p', { className: 'smgr-toast', role: 'status' }, notice) : null, error !== null ? h('p', { className: 'smgr-error', role: 'alert' }, error) : null, data === null && !error ? h('p', { className: 'smgr-empty' }, '正在加载…') : null, data
                    ? data.roots.map(function (root) {
                        ...
                        return h('section', { key: root.id, className: 'smgr-group' }, h('h3', null, root.label, h('span', { className: 'smgr-count' }, String(root.skills.length)), shadowCount > 0
                            ? h('span', { className: 'smgr-badge smgr-badgeShadow', title: '项目级禁用开关文件：默认隐藏，搜索可列出' }, '禁用开关 ×' + shadowCount)
                            : null, !root.exists ? '（目录尚不存在，导入/编辑时自动创建）' : ''), items.length === 0 ? h('p', { className: 'smgr-empty' }, root.exists ? (q === '' && shadowCount > 0 ? '' : '（空）') : '') : null, items);
                    })
                    : null, data
                    ? (data.bundled || []).map(function (group) {
                        ...
                        return h('section', { key: 'bundled:' + group.presetId, className: 'smgr-group' }, h('h3', null, group.label, h('span', { className: 'smgr-count' }, String(group.skills.length))), items.length === 0 ? h('p', { className: 'smgr-empty' }, '（空）') : null, items);
                    })
                    : null, sel !== null
                    ? detail === null
                        ? h('section', { className: 'smgr-detail' }, h('p', { className: 'smgr-empty' }, '正在读取…'))
                        : h('section', { className: 'smgr-detail' }, h('div', { className: 'smgr-detailHead' }, h('h3', null, detail.name), h('code', { className: 'smgr-path' }, detail.path), h('div', { className: 'smgr-detailActions' }, h(Button, { variant: 'outline', onClick: function () { exportSkill({ name: detail.name }, detail.content); } }, '导出'), !detail.readOnly
                            ? h(Button, { variant: 'outline', className: 'smgr-iconBtnDanger', onClick: function () { setConfirmDelete(true); } }, '删除')
                            : null)), h('pre', { className: 'smgr-code' }, detail.content))
                    : null, h(Modal, {
                    open: importOpen,
                    ...
                    footer: h(React.Fragment, null, h(Button, { variant: 'outline', onClick: function () { setImportOpen(false); } }, '取消'), h(Button, { disabled: busy || importText.trim() === '', onClick: doImport }, busy ? '导入中…' : '导入'))
                }, h('div', { className: 'smgr-modalBody' }, h('label', { className: 'smgr-file' }, '目标根目录：', h('select', { className: 'smgr-select', value: importRoot, onChange: function (event) { setImportRoot(event.target.value); } }, editableRoots.map(function (root) {
                    return h('option', { key: root.id, value: root.id }, root.label);
                }))), h('input', { type: 'file', accept: '.md,.markdown', className: 'smgr-file', onChange: onImportFile }), h('textarea', { className: 'smgr-editor', value: importText, spellCheck: false, placeholder: '---\nname: my-skill\ndescription: 一句话说明这个 skill 做什么\n---\n\n正文指令…', onChange: function (event) { setImportText(event.target.value); } }))), h(Modal, {
                    open: confirmDelete,
                    onClose: function () { setConfirmDelete(false); },
                    title: '删除 skill？',
                    closeLabel: '关闭',
                    description: '将从磁盘删除该 skill 文件（目录型 skill 连同整个目录），此操作不可撤销。',
                    footer: h(React.Fragment, null, h(Button, { variant: 'outline', onClick: function () { setConfirmDelete(false); } }, '取消'), h(Button, { className: 'smgr-iconBtnDanger', disabled: busy, onClick: doDelete }, busy ? '删除中…' : '确认删除'))
                }, sel !== null ? h('p', { style: { margin: 0, fontSize: 13 } }, '删除 ', h('strong', null, sel.name), '？') : null), h(Modal, {
                    open: confirmPkg !== null,
                    onClose: function () { setConfirmPkg(null); },
                    title: '删除整个技能包？',
                    closeLabel: '关闭',
                    description: '将逐个删除该技能包的全部成员（目录型 skill 连同整个目录），此操作不可撤销。',
                    footer: h(React.Fragment, null, h(Button, { variant: 'outline', onClick: function () { setConfirmPkg(null); } }, '取消'), h(Button, { className: 'smgr-iconBtnDanger', disabled: busy, onClick: doPkgDelete }, busy ? '删除中…' : '确认删除全部'))
                }, confirmPkg !== null
                    ? h('div', null, h('p', { style: { margin: '0 0 6px', fontSize: 13 } }, '删除「', h('strong', null, confirmPkg.label), '」的全部 ', String(confirmPkg.names.length), ' 个 skill：'), h('div', { className: 'smgr-pkgList' }, confirmPkg.names.join('\n')))
                    : null));
            }
            // ── DSH-008 V1: per-project SKILL management ─────────────────
            ...
            function buildProjectOptions(ctx) {
                ...
                function push(cwd, title, kind) {
                    if (typeof cwd !== 'string' || cwd === '' || seen[cwd] === true)
                        return;
                    seen[cwd] = true;
                ...
                try {
                    ...
                    if (snap && Array.isArray(snap.items)) {
                        ...
                        for (var i = 0; i < items.length && out.length < 12; i += 1) {
                            if (items[i] && typeof items[i].path === 'string')
                                push(items[i].path, items[i].title, 'workspace');
                        }
                    }
                }
            ...
            function DiffView(props) {
                ...
                function group(title, key, values, renderRow) {
                    if (!Array.isArray(values) || values.length === 0)
                        return;
                    groups.push(h('section', { key: key, className: 'sk-diffGroup', 'aria-label': title }, h('div', { className: 'sk-diffGroupHead' }, title, h('span', { className: 'sk-diffGroupCount' }, String(values.length))), values.map(renderRow)));
                }
                group('将启用', 'enable', diff.toEnable || [], function (n) {
                    return h('div', { key: 'e' + n, className: 'sk-diffRow' }, h('span', { className: 'sk-diffName' }, n), h('span', { className: 'sk-diffState sk-diffStateAdd' }, '启用'));
                });
                group('将停用', 'disable', diff.toDisable || [], function (n) {
                    return h('div', { key: 'd' + n, className: 'sk-diffRow' }, h('span', { className: 'sk-diffName' }, n), h('span', { className: 'sk-diffState sk-diffStateDel' }, '停用'));
                });
                group('来源变更', 'source', diff.sourceChanges || [], function (c) {
                    return h('div', { key: 's' + c.name, className: 'sk-diffRow' }, h('span', { className: 'sk-diffName' }, c.name), h('span', { className: 'sk-diffMeta' }, (c.from || '默认') + ' → ' + (c.to || '默认')));
                });
                if (groups.length === 0)
                    groups.push(h('p', { key: 'none', className: 'sk-slimNote' }, '没有变化：当前配置已与该预设一致'));
                if (!props || props.showFinal !== false)
            ...
            function SkillMarketplacePage(props) {
                ...
                var [githubInfo, setGithubInfo] = React.useState(null);
                var [githubPath, setGithubPath] = React.useState('');
                var [githubPreview, setGithubPreview] = React.useState(null);
                var [githubBusy, setGithubBusy] = React.useState(false);
                var [githubError, setGithubError] = React.useState(null);
                ...
                function inspectGithub() {
                    if (githubUrl.trim() === '' || githubBusy)
                        return;
                    setGithubBusy(true);
                    setGithubError(null);
                    setGithubInfo(null);
                    setGithubPreview(null);
                    apiCallAt('github.inspect', { url: githubUrl.trim() }, ctx).then(function (value) {
                        setGithubInfo(value);
                    ...
                    }, function (reason) { setGithubError(String((reason && reason.message) || reason)); }).finally(function () { setGithubBusy(false); });
                }
                function previewGithub() {
                    if (project === null) {
                        setGithubError('请先选择安装目标项目');
                        return;
                    }
                    if (githubPath === '' || githubBusy)
                        return;
                    setGithubBusy(true);
                    setGithubError(null);
                    apiCallAt('github.preview', { cwd: project.cwd, url: githubUrl.trim(), path: githubPath }, ctx).then(function (value) { setGithubPreview(value); }, function (reason) { setGithubError(String((reason && reason.message) || reason)); }).finally(function () { setGithubBusy(false); });
                }
                function installGithub() {
                    if (project === null || githubPreview === null || !githubPreview.canInstall || githubBusy)
                        return;
                    setGithubBusy(true);
                    setGithubError(null);
                    apiCallAt('github.install', { cwd: project.cwd, url: githubUrl.trim(), path: githubPath }, ctx).then(function () {
                        setGithubOpen(false);
                        setGithubPreview(null);
                        setGithubInfo(null);
                        setGithubUrl('');
                ...
                return h('div', { className: 'sk-marketLayout' }, h('div', { className: 'sk-marketListCol' }, h('div', { className: 'sk-marketToolbar' }, h('span', { className: 'sk-projLabel' }, '安装目标'), h('div', { className: 'sk-marketProjectPicker' }, h('button', {
                    ...
                    : null), h('div', { className: 'sk-searchWrap sk-marketSearch' }, h('span', { className: 'sk-ic' }, h(P.IconSearchOutline16)), h('input', { className: 'sk-search', placeholder: '搜索 Skill 或 GitHub 仓库', value: query, onChange: function (event) { setQuery(event.target.value); } })), h('button', { type: 'button', className: 'sk-chip', disabled: project === null, onClick: function () { setGithubOpen(true); setGithubError(null); } }, h(P.IconPlusOutline16), '从 GitHub 安装'), h('button', { type: 'button', className: 'sk-chip', disabled: loading, onClick: function () { setAttempt(function (value) { return value + 1; }); } }, h(P.IconRefreshOutline16), loading ? '刷新中…' : '刷新')), h('p', { className: 'sk-marketHelper' }, '精选或任意公开 GitHub 仓库 · 安装阶段不执行第三方代码；Skill 被调用后仍可能请求执行仓库脚本，并受 Agent 权限与审批约束'), message ? h('p', { className: 'sk-marketNotice', role: 'status' }, message) : null, error ? h('div', { className: 'sk-marketLoadError', role: 'alert' }, h('p', { className: 'sk-error' }, '加载 Skill 市场失败：' + error), h(Button, { variant: 'outline', onClick: function () { setAttempt(function (value) { return value + 1; }); } }, '重试')) : null, h('div', { className: 'sk-marketList', 'data-testid': 'skill-market-list' }, loading && !loaded ? h('p', { className: 'sk-empty', role: 'status' }, '正在读取 Skill 市场…') : null, !loading && visible.length === 0 ? h('p', { className: 'sk-empty' }, '没有匹配的 Skill。') : null, visible.map(function (item) {
                    ...
                    open: githubOpen,
                    onClose: function () { if (!githubBusy) {
                        setGithubOpen(false);
                        setGithubPreview(null);
                        setGithubError(null);
                    } },
                    ...
                    footer: h(React.Fragment, null, h(Button, { variant: 'outline', disabled: githubBusy, onClick: function () { setGithubOpen(false); setGithubPreview(null); } }, '取消'), githubPreview ? h(Button, { disabled: githubBusy || !githubPreview.canInstall, onClick: installGithub }, githubBusy ? '写入中…' : githubPreview.canInstall ? '确认安装' : '无法安装') : h(Button, { disabled: githubBusy || githubPath === '', onClick: previewGithub }, githubBusy ? '校验中…' : '生成安装预览'))
                }, h('div', { className: 'sk-marketPreview', 'data-testid': 'github-install-dialog' }, h('div', { className: 'sk-presetField' }, h('label', { className: 'sk-presetFieldHead' }, h('span', null, 'GitHub URL')), h('div', { className: 'sk-presetInputRow' }, h('input', { className: 'sk-presetInput', value: githubUrl, placeholder: 'https://github.com/owner/repo', disabled: githubBusy || githubPreview !== null, onChange: function (event) { setGithubUrl(event.target.value); setGithubInfo(null); setGithubPath(''); } }), h(Button, { variant: 'outline', disabled: githubBusy || githubUrl.trim() === '' || githubPreview !== null, onClick: inspectGithub }, githubBusy ? '读取中…' : '检查仓库'))), githubInfo ? h('div', { className: 'sk-presetField' }, h('label', { className: 'sk-presetFieldHead' }, h('span', null, 'Skill 目录')), h('select', { className: 'sk-presetInput', value: githubPath, disabled: githubBusy || githubPreview !== null, onChange: function (event) { setGithubPath(event.target.value); } }, h('option', { value: '' }, '请选择…'), (githubInfo.candidates || []).map(function (candidate) { return h('option', { key: candidate.path, value: candidate.path }, candidate.path); }))) : null, githubError ? h('p', { className: 'sk-error', role: 'alert' }, githubError) : null, githubPreview ? h('div', { className: 'sk-marketPreviewSummary' }, h('strong', null, githubPreview.action === 'update' ? '将更新受管 Skill' : '将安装 ' + githubPreview.name), h('span', null, githubPreview.repository + ' · ' + githubPreview.path + ' @ ' + githubPreview.ref), h('span', null, githubPreview.incoming.fileCount + ' 个文件 · ' + githubPreview.incoming.hash), h('span', null, githubPreview.message), h('span', null, '安装阶段不执行第三方代码；Skill 调用后的脚本行为仍受 Agent 权限与审批约束。')) : null)));
            }
            function SkillFindingState() {
                ...
                return h('div', { className: 'sk-findingState', role: 'status', 'aria-live': 'polite', 'aria-atomic': 'true' }, h('div', { className: 'sk-findingVisual', 'aria-hidden': true }, particles.map(function (particle) {
                    return h('span', {
                        key: String(particle[0]),
                        className: 'sk-findingParticle sk-findingParticle' + particle[0]
                            + (particle[1] ? ' sk-findingParticleAccent' : '')
                    });
                }), h('span', { className: 'sk-findingCore' }, h(P.IconSkillOutline16))), h('span', { className: 'sk-findingLabel', 'data-text': 'Skill Finding' }, 'Skill Finding', h('span', { className: 'sk-findingCursor', 'aria-hidden': true })));
            }
            /**
            ...
            function SkillCenterV1Body(props) {
                ...
                var loadView = React.useCallback(function (proj) {
                    ...
                    setViewBusy(true);
                    setViewError(null);
                    return apiCallAt('catalog', proj ? { cwd: proj.cwd } : {}, ctx).then(function (value) {
                ...
                var loadPresets = React.useCallback(function () {
                    return apiCallAt('presets.list', {}, ctx).then(function (value) { setPresets(Array.isArray(value && value.presets) ? value.presets : []); }, function () { });
                }, [ctx]);
                ...
                React.useEffect(function () {
                    ...
                    var stored = null;
                    try {
                        stored = window.localStorage.getItem('smgr.v1.project');
                    }
                    catch (error) { }
                    var pick = null;
                ...
                React.useEffect(function () {
                    ...
                    function onKey(event) {
                        ...
                        for (var i = 0; i < dialogs.length; i += 1) {
                            if (dialogs[i].classList.contains('sk-drawer') === false)
                                return;
                        }
                        setDrawerName(null);
                        setTagDraft('');
                ...
                function chooseProject(p) {
                    ...
                    if (p === null || (project !== null && project.cwd === p.cwd))
                        return;
                    ...
                    setViewBusy(true);
                    try {
                        window.localStorage.setItem('smgr.v1.project', p.cwd);
                    }
                    catch (error) { }
                }
                ...
                function doToggle(row, force) {
                    var proj = project;
                    if (proj === null || toggling[row.name] === true || viewBusy === true)
                        return;
                    if (view && (view.configCorrupt === true || view.configFuture === true))
                        return;
                    ...
                    apiCallAt('setEnabled', { cwd: proj.cwd, name: row.name, enabled: want }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj.cwd)) {
                            return;
                        }
                        ...
                        if (value && value.partial === true) {
                            ...
                            for (var pi = 0; pi < failed.length; pi += 1) {
                                parts.push((failed[pi].name || '*') + '（' + (failed[pi].error || '失败') + '）');
                            }
                            for (var pj = 0; pj < conflicts.length; pj += 1) {
                                parts.push((conflicts[pj].name || '*') + '（' + (conflicts[pj].message || '冲突') + '）');
                            }
                ...
                function doBulk(enabled) {
                    ...
                    if (names.length === 0 || proj === null || viewBusy === true)
                        return;
                    if (view && (view.configCorrupt === true || view.configFuture === true))
                        return;
                    var gen = genRef.current;
                    setViewError(null);
                    apiCallAt('setMany', { cwd: proj.cwd, names: names, enabled: enabled }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj.cwd)) {
                            return;
                        }
                        setSelectedRows({});
                        ...
                        void loadView(proj);
                    }, function (e) { if (isCurrentProject(gen, proj.cwd)) {
                        setViewError(String((e && e.message) || e));
                    } });
                }
                function doSource(name, sourceKey) {
                    var proj = project;
                    if (proj === null || sourceBusy === name || viewBusy === true)
                        return;
                    if (view && (view.configCorrupt === true || view.configFuture === true))
                        return;
                    var gen = genRef.current;
                    setSourceBusy(name);
                    setViewError(null);
                    apiCallAt('setSource', { cwd: proj.cwd, name: name, source: sourceKey }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj.cwd)) {
                            return;
                        }
                        setSourceBusy(null);
                ...
                function doTags(row, tags) {
                    var proj = project;
                    if (viewBusy === true || tagBusy === true)
                        return;
                    var gen = genRef.current;
                    setTagBusy(true);
                    setViewError(null);
                    apiCallAt('setTags', { cwd: proj ? proj.cwd : undefined, name: row.name, tags: tags }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj ? proj.cwd : '')) {
                            setTagBusy(false);
                            return;
                        }
                        if (value && value.view) {
                ...
                function commitTag() {
                    var row = drawerRow;
                    var v = tagDraft.trim();
                    if (row === null || v === '' || tagBusy === true)
                        return;
                    var tags = (row.tags || []).slice();
                    if (tags.length >= 20 || tags.indexOf(v) !== -1)
                        return;
                    tags.push(v);
                    doTags(row, tags);
                }
                ...
                function openPreset(p) {
                    var proj = project;
                    if (proj === null) {
                        setViewError('请先选择项目，再应用预设');
                        return;
                    }
                    var gen = genRef.current;
                    setViewError(null);
                    apiCallAt('presets.preview', { cwd: proj.cwd, name: p.name, mode: 'replace' }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj.cwd))
                            return;
                ...
                function switchPresetMode(mode) {
                    ...
                    var proj = project;
                    if (modal === null || proj === null || modal.cwd !== proj.cwd)
                        return;
                    var gen = genRef.current;
                    apiCallAt('presets.preview', { cwd: proj.cwd, name: modal.name, mode: mode }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj.cwd))
                            return;
                ...
                function applyPreset() {
                    var proj = project;
                    if (presetModal === null || proj === null || presetModal.cwd !== proj.cwd || presetBusy || viewBusy === true)
                        return;
                    if (view && (view.configCorrupt === true || view.configFuture === true))
                        return;
                    var gen = genRef.current;
                    setPresetBusy(true);
                    setViewError(null);
                    apiCallAt('presets.apply', { cwd: proj.cwd, name: presetModal.name, mode: presetModal.mode }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj.cwd)) {
                            return;
                        }
                        setPresetBusy(false);
                ...
                function setDefaultPreset(name) {
                    apiCallAt('presets.setDefault', { name: name }, ctx).then(function () { void loadPresets(); }, function (e) { setViewError(String((e && e.message) || e)); });
                }
                function deletePreset(name) {
                    apiCallAt('presets.delete', { name: name }, ctx).then(function () { setPresetModal(null); void loadPresets(); }, function (e) { setViewError(String((e && e.message) || e)); });
                }
                function savePreset() {
                    ...
                    setPresetBusy(true);
                    setViewError(null);
                    apiCallAt('presets.save', { cwd: proj.cwd, name: name, description: saveDesc.trim() === '' ? undefined : saveDesc.trim() }, ctx).then(function () {
                        if (!isCurrentProject(gen, proj.cwd))
                            return;
                        setPresetBusy(false);
                        setSaveOpen(false);
                ...
                function doSlimPreview() {
                    ...
                    var gen = genRef.current;
                    setViewError(null);
                    apiCallAt('slim.preview', { cwd: proj.cwd }, ctx).then(function (value) { if (isCurrentProject(gen, proj.cwd))
                        setSlimModal(Object.assign({}, value, { cwd: proj.cwd, gen: gen })); }, function (e) { if (isCurrentProject(gen, proj.cwd))
                        setViewError(String((e && e.message) || e)); });
                }
                function doSlimApply() {
                    var proj = project;
                    if (proj === null || slimModal === null || slimModal.cwd !== proj.cwd || slimBusy || viewBusy === true)
                        return;
                    if (view && (view.configCorrupt === true || view.configFuture === true))
                        return;
                    var gen = genRef.current;
                    setSlimBusy(true);
                    setViewError(null);
                    apiCallAt('slim.apply', { cwd: proj.cwd }, ctx).then(function (value) {
                        if (!isCurrentProject(gen, proj.cwd)) {
                            return;
                        }
                        setSlimBusy(false);
                ...
                if (view !== null) {
                    ...
                    if (enableFilter === 'on')
                        visibleRows = visibleRows.filter(function (r) { return r.enabled === true; });
                    if (enableFilter === 'off')
                        visibleRows = visibleRows.filter(function (r) { return r.enabled !== true; });
                    if (tagFilter !== null) {
                ...
                for (var visibleIndex = 0; visibleIndex < visibleRows.length; visibleIndex += 1) {
                    if (selectedRows[visibleRows[visibleIndex].name] === true)
                ...
                function rowEl(row) {
                    ...
                    }, h('div', { className: 'sk-rowMain', title: row.description || undefined }, h('div', { className: 'sk-rowTitle' }, h('span', { className: 'sk-rowName' }, row.name), row.specialized === true
                        ? h('span', { className: 'sk-badge sk-badgeSpec', title: '本项目存在已修改的来源副本（项目特化）' }, '项目特化')
                        : null, row.updateInfo !== null && row.updateInfo !== undefined
                        ? h('span', { className: 'sk-badge sk-badgeUpdate', title: String(row.updateInfo) }, '可更新')
                        : null, row.sources.length > 1
                        ? h('span', { className: 'sk-badge', title: '该 Skill 存在多个来源，可在详情中选择' }, '来源 ×' + row.sources.length)
                        : null), row.description ? h('div', { className: 'sk-rowDesc' }, firstSentence(row.description)) : null)), h('div', { className: 'sk-rowSide' }, !bulkMode && toggling[row.name] === true ? h('span', { className: 'sk-saving', role: 'status' }, '保存中') : null, !bulkMode ? switchV1(row) : null));
                }
                function drawerEl() {
                    var row = drawerRow;
                    if (row === null)
                    ...
                    for (var i = 0; i < row.sources.length; i += 1) {
                        if (row.sources[i].key === row.effectiveSourceKey) {
                            eff = row.sources[i];
                            break;
                        }
                    }
                    ...
                    for (var j = 0; j < row.sources.length; j += 1) {
                        if (row.sources[j].key === row.defaultSourceKey) {
                            defaultSrc = row.sources[j];
                            break;
                        }
                    }
                    ...
                    var tagIssue = tagAtLimit
                        ...
                        : tagDuplicate
                            ? '这个标签已经存在'
                            : '按 Enter 添加';
                    return h('aside', { className: 'sk-drawer', role: 'dialog', 'aria-label': 'Skill 详情：' + row.name }, h('div', { className: 'sk-drawerHead' }, h('div', { className: 'sk-drawerName' }, row.name), h('span', { className: 'sk-badge' + (row.enabled === true ? ' sk-badgeSpec' : '') }, row.enabled === true ? '已启用' : '未启用'), h('button', {
                        type: 'button',
                        className: 'sk-icBtn',
                        'aria-label': '关闭详情',
                        title: '关闭（Esc）',
                        onClick: function () { setDrawerName(null); setTagDraft(''); setTagComposerOpen(false); setAdvOpen(false); setSourceOpen(false); }
                    }, h(P.IconCloseOutline16))), h('div', { className: 'sk-drawerBody' }, h('div', { className: 'sk-drawerRow' }, h('div', null, h('div', { className: 'sk-drawerRowLabel' }, '启用此 Skill'), h('div', { className: 'sk-slimNote' }, '关闭后模型在本项目不再自动调用，仍可用 /' + row.name + ' 手动调用；下一轮对话生效')), switchV1(row)), h('div', { className: 'sk-sec' }, h('div', { className: 'sk-secTitle' }, '描述'), row.description
                        ? h('p', { className: 'sk-descFull' }, row.description)
                        : h('p', { className: 'sk-slimNote' }, '（无描述）')), h('div', { className: 'sk-sec' }, h('div', { className: 'sk-secTitle' }, '来源'), h('div', { className: 'sk-sourceSummary' }, h('div', { className: 'sk-sourceCurrent' }, eff
                        ? h('div', { className: 'sk-srcName' }, eff.label, row.specialized === true && eff.key === row.effectiveSourceKey
                            ? h('span', { className: 'sk-badge sk-badgeSpec' }, '项目副本')
                            : null)
                        : h('span', { className: 'sk-slimNote' }, '无可用来源'), eff ? h('span', { className: 'sk-srcMeta' }, eff.scope === 'project' ? '项目来源' : eff.scope === 'bundled' ? '内置来源' : '用户来源') : null), row.sources.length > 0
                        ? h('button', {
                            type: 'button',
                        ...
                        : null), sourceOpen && row.sources.length > 0
                        ? h('div', { className: 'sk-srcList', role: 'radiogroup', 'aria-label': row.name + ' 的来源' }, h('button', {
                            type: 'button',
                            role: 'radio',
                        ...
                        }, h('span', { className: 'sk-radio' + (row.sourceKey === null ? ' sk-radioOn' : '') }), h('span', { className: 'sk-srcMain' }, h('span', { className: 'sk-srcName' }, '默认（按优先级自动选择）'), h('span', { className: 'sk-srcMeta' }, defaultSrc ? '当前解析为：' + defaultSrc.label : '无可用来源'))), row.sources.map(function (s) {
                            ...
                            }, h('span', { className: 'sk-radio' + (on ? ' sk-radioOn' : '') }), h('span', { className: 'sk-srcMain' }, h('span', { className: 'sk-srcName' }, s.label, sourceIsBroken(s.broken) ? h('span', { className: 'sk-badge sk-badgeWarn' }, '格式损坏') : null, s.modified === true ? h('span', { className: 'sk-badge sk-badgeSpec' }, '已修改') : null, s.stale === true ? h('span', { className: 'sk-badge', title: '通用来源内容已变化' }, '来源有更新') : null), h('span', { className: 'sk-srcMeta' }, s.scope === 'project' ? '项目' : s.scope === 'global' ? '其他全局（只读）' : s.scope === 'bundled' ? '内置（只读）' : '用户级', ' · 优先级 ' + s.rank)));
                        }))
                        : null), h('div', { className: 'sk-sec' }, h('div', { className: 'sk-tagHeading' }, h('div', { className: 'sk-secTitle' }, '标签'), h('span', { className: 'sk-tagScope' }, '全局共享')), tagValues.length > 0
                        ? h('div', { className: 'sk-tagList sk-tagListBare', 'aria-label': '已有标签' }, tagValues.map(function (t) {
                            return h('span', { key: t, className: 'sk-tag' }, t, h('button', {
                                type: 'button',
                                className: 'sk-tagX',
                                'aria-label': '移除标签「' + t + '」',
                                title: '移除标签「' + t + '」',
                    ...
                    }, tagComposerOpen ? h(P.IconChevronDownOutline14, { style: { transform: 'rotate(180deg)' } }) : h(P.IconPlusOutline16), tagComposerOpen ? '收起标签输入' : '添加标签'), tagComposerOpen
                        ? h('div', { className: 'sk-tagPanel' }, h('div', { className: 'sk-tagComposer' }, h('input', {
                            ...
                            disabled: !tagCanAdd,
                            onClick: commitTag
                        }, tagBusy ? '保存中…' : '添加')), h('div', {
                            id: tagHelpId,
                            className: 'sk-tagFoot' + ((tagAtLimit || tagDuplicate) ? ' sk-tagIssue' : ''),
                            'aria-live': 'polite'
                        }, h('span', null, tagIssue), h('span', null, tagValues.length + '/20 · 每个最多 32 字符')))
                        : null), h('div', { className: 'sk-adv' }, h('button', {
                        type: 'button',
                        className: 'sk-advBtn',
                        onClick: function () { setAdvOpen(!advOpen); }
                    }, h(P.IconChevronDownOutline14, { style: { transform: advOpen ? 'rotate(180deg)' : 'none', transition: 'transform .12s' } }), '更多信息'), advOpen
                        ? h('div', { className: 'sk-advBody' }, h('div', null, '当前来源路径：', h('code', null, (eff && eff.path) || '—')), view && view.projectRoot ? h('div', null, '项目配置：', h('code', null, view.projectRoot + '/.dsh/skill-manager.json'), '（仅本机，不提交 Git）') : null, h('div', null, '格式：', eff ? (eff.format === 'dir' ? '目录（附属文件 ' + eff.files.length + ' 个）' : '单文件') : '—'), h('div', null, '更新：', row.updateInfo !== null && row.updateInfo !== undefined ? String(row.updateInfo) : '当前版本不检测远端更新'), h('div', null, '项目特化：V1.2 后续能力，当前版本不可用'), eff && Array.isArray(eff.files) && eff.files.length > 0
                            ? h('div', null, '附属文件：', h('code', null, eff.files.join(', ')))
                            : null, view && view.configCorrupt === true
                            ? h('div', { style: { color: 'var(--dsw-alias-state-warn-primary)' } }, '注意：项目配置文件已损坏，按空配置处理（可手动删除后重建）')
                            : null, view && view.configExisted === false
                ...
                if (view !== null) {
                    for (var k = 0; k < view.identities.length; k += 1) {
                        if (view.identities[k].enabled === true)
                            enabledCount += 1;
                    }
                }
                ...
                for (var presetIndex = 0; presetIndex < presets.length; presetIndex += 1) {
                    if (presets[presetIndex].defaultSlim === true) {
                        recommendedPreset = presets[presetIndex];
                        break;
                    }
                }
            ...
            function SkillCenterV1(props) {
                ...
                React.useEffect(function () {
                    ...
                    function accept(value) {
                        if (value && Number(value.apiVersion) >= 6)
                            setProbe('v1');
                        else
                            setProbe('legacy');
                    }
                    function reject(e) {
                        ...
                        apiCallAt('catalog', {}, ctx).then(function (value) { accept(value); }, function (catalogError) {
                            var catalogMessage = String((catalogError && catalogError.message) || catalogError);
                            if (catalogMessage.indexOf('未知操作') === 0)
                                setProbe('legacy');
                            else {
                                setProbe('error');
                                setProbeError(catalogMessage);
                            }
                        });
                    }
                    apiCallAt('capabilities', {}, ctx).then(accept, function (e) {
                        reject(e);
                    });
                }, [attempt, ctx]);
                if (probe === 'loading') {
                ...
                if (probe === 'legacy') {
                    return h('div', { className: 'smgr' }, h('p', { className: 'sk-banner' }, '当前 dsh web 尚未加载 DSH-008 的 host 接口（apiVersion 6）。重启 dsh web 后即可使用新版项目 SKILL 管理中心；现在显示旧版界面。'), h(SkillManagerSection, { api: api }));
                }
                if (probe === 'error') {
                    return h('div', { className: 'sk-root' }, h('p', { className: 'sk-error' }, '加载 SKILL 管理中心失败：' + probeError), h(Button, { variant: 'outline', onClick: function () { setAttempt(function (n) { return n + 1; }); } }, '重试'));
                }
