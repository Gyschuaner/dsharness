    ...
    window.__ModuleLoader__.load({
        ...
        factory: function (require) {
            ...
            function SkillMarketplacePage(props) {
                ...
                var [marketWarning, setMarketWarning] = React.useState(null);
                ...
                function loadMarket(force, sort = marketSort) {
                    setLoading(true);
                    setError(null);
                    return apiCallAt('marketplace', { cwd: project ? project.cwd : undefined, force: force === true, sort: sort }, ctx).then(function (value) {
            ...
            function SkillFindingState(props) {
                ...
                return h('div', { className: 'sk-findingState', role: 'status', 'aria-live': 'polite', 'aria-atomic': 'true', 'aria-label': ariaLabel }, h('div', { className: 'sk-findingVisual', 'aria-hidden': true }, particles.map(function (particle) {
                    return h('span', {
                        key: String(particle[0]),
                        className: 'sk-findingParticle sk-findingParticle' + particle[0]
                            + (particle[1] ? ' sk-findingParticleAccent' : '')
                    });
                }), h('span', { className: 'sk-findingCore' }, h(P.IconSkillOutline16))), h('span', { className: 'sk-findingLabel', 'data-text': label }, label, h('span', { className: 'sk-findingCursor', 'aria-hidden': true })));
            }
            /**
            ...
            function SkillCenterV1(props) {
                ...
                if (probe === 'unavailable') {
                    return h('div', { className: 'sk-root' }, h('p', { className: 'sk-error' }, '当前 dsh web 未加载新版 Skill Host API（apiVersion 6），请重启 dsh web 后重试。'), h(Button, { variant: 'outline', onClick: function () { setAttempt(function (n) { return n + 1; }); } }, '重试'));
                }
                if (probe === 'error') {
