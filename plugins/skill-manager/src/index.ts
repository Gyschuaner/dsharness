...
interface ApiBody extends UnknownRecord {
	op?: string;
	cwd?: string;
	root?: string;
	name?: string | null;
...
interface HostContext {
	logger?: Logger;
	get(service: 'webServer'): { register(route: { kind: 'exact'; path: string; handler: ReturnType<typeof makeHandler> }): unknown } | undefined;
...
async function readPolicyState(opts?: CatalogOptions): Promise<PolicyState> {
	const { config } = await readGlobalConfig(opts);
	return { globalDefaultOff: config.globalDefaultOff === true };
}
async function writePolicyState(state: PolicyState, opts?: CatalogOptions): Promise<void> {
 ...
 */
async function policyWrite(path: string, content: string): Promise<void> {
...
async function findUserSkill(cwd: string | undefined, skillName: string, opts?: CatalogOptions): Promise<TargetPlan | undefined> {
...
async function findProjectSkill(cwd: string, skillName: string, opts?: CatalogOptions): Promise<{ plan: TargetPlan; marker: boolean } | undefined> {
	const { roots } = await computeRoots(cwd, opts);
	for (const root of roots) {
 ...
 */
async function enforceGlobalPolicy(cwd: string | undefined, opts?: CatalogOptions): Promise<{ changed: number; markersRemoved: number; failed: Array<{ name: string; path: string; error: string }> }> {
	const state = await readPolicyState(opts);
	if (!state.globalDefaultOff) return { changed: 0, markersRemoved: 0, failed: [] };
	const { roots } = await computeRoots(cwd, opts);
	let changed = 0;
	const failed: Array<{ name: string; path: string; error: string }> = [];
	for (const root of roots) {
		if (root.scope !== 'user') continue;
 ...
 *   'dir' | 'flat' | undefined.
 */
async function resolveTarget(agentPresets: AgentPresetService | undefined, cwd: string | undefined, rootId: unknown, skillName: unknown, forCreate: boolean, opts?: CatalogOptions): Promise<TargetPlan> {
	...
	if (typeof rootId === 'string' && rootId.startsWith('bundled:')) {
		...
		let preset: UnknownRecord | undefined;
		try {
			preset = await agentPresets?.list().then((list) => list.find((p): p is UnknownRecord => p !== null && typeof p === 'object' && 'id' in p && p.id === presetId));
		} catch {
			preset = undefined;
		}
		...
		const plan: TargetPlan = { rootId, readOnly: true, readOnlyReason: 'bundled', dir, path: '' };
...
async function canonicalPathAllowMissing(path: string): Promise<string> {
	...
	for (;;) {
		try {
			...
			if (parent === cursor) throw new ApiError(400, '无法解析目标路径');
...
async function assertContained(plan: Pick<TargetPlan, 'dir' | 'path'>): Promise<void> {
	...
	if (rel === '' || isAbsolute(rel) || rel === '..' || rel.startsWith(`..${sep}`)) throw new ApiError(400, '非法路径');
}
 ...
 */
function presetDiff(currentConfig: ProjectConfig, preset: SkillPreset, _identities: unknown, mode: 'replace' | 'merge') {
	...
	const sourceChanges: Array<{ name: string; from: string | null; to: string | null }> = [];
...
async function buildProjectViewLocked(cwd: string | undefined, opts?: CatalogOptions) {
	if (typeof cwd !== 'string' || cwd.length === 0) return buildProjectView(cwd, opts, { reconcile: false });
...
async function projectRootSignature(cwd: string, opts?: CatalogOptions): Promise<string> {
	const { roots } = await computeRoots(cwd, opts);
...
function sameTargetSourceConfig(left: ProjectConfig, right: ProjectConfig, skillName: string): boolean {
	const fingerprint = (entry: unknown) => JSON.stringify(entry === null || typeof entry !== 'object'
		? entry
 ...
 */
function canUseTargetToggle(identity: SkillIdentity | undefined, config: ProjectConfig, skillName: string): boolean {
...
async function cachedIdentityStillCurrent(snapshot: ProjectSnapshot, cwd: string, opts: CatalogOptions, identity: SkillIdentity): Promise<boolean> {
 ...
 */
async function mutateProject(cwd: string, opts: CatalogOptions, fn: (context: MutateContext) => Promise<void> | void) {
	const projectRoot = await findProjectRoot(cwd);
 ...
 */
function targetReport(skillNames: string | string[], report: ReconcileReport): boolean {
	const problems: Array<{ name: string; error?: string; message?: string }> = [...report.failed, ...report.conflicts];
...
function configPayload(view: ProjectView, config: ProjectConfig) {
	return {
 ...
 */
function makeHandler(deps: HandlerDeps): RequestHandler {
	...
	async function tryTargetToggle(cwd: string, skillName: string, enabled: boolean) {
		const projectRoot = await findProjectRoot(cwd);
		return withConfigLock(projectLockKey(projectRoot), async () => {
			...
			if (read.existed === false) return { value: null, reason: 'baseline-not-persisted' };
			...
			if (identity === undefined || !(await cachedIdentityStillCurrent(snapshot, cwd, opts, identity))) return { value: null, reason: 'source-tree-changed' };
	...
	async function installWithProjectBaseline(cwd: string | undefined, install: () => Promise<unknown>) {
		const checkedCwd = await assertCwd(cwd);
		if (checkedCwd === undefined) throw new ApiError(400, '安装 Skill 前请先选择当前项目');
		...
		return withConfigLock(projectLockKey(projectRoot), async () => {
			...
			if (built.view.configCorrupt === true) throw new ApiError(409, `项目配置已损坏：${projectConfigPath(projectRoot)}`);
			if (built.view.configFuture === true) throw new ApiError(409, `项目配置版本高于 Host ${PROJECT_API_VERSION}`);
			...
			try {
				...
				if (baselineCreated) await unlink(projectConfigPath(projectRoot)).catch(() => {});
				throw error;
			}
		});
	}
	const ops: Record<string, (body: ApiBody) => Promise<unknown>> = {
		async capabilities() {
			return {
				apiVersion: PROJECT_API_VERSION,
		...
		async ['github.inspect'](body) {
			if (typeof body.url !== 'string' || body.url.trim() === '') throw new ApiError(400, '缺少 GitHub URL');
		...
		async list(body) {
			...
			const out: Array<{ id: string; scope: string; label: string; dir: string; rank: number; exists: boolean; skills: SkillFile[] }> = [];
			for (const root of roots) {
				const result = await discoverInRoot(root.dir);
			...
			const byName = new Map<string, Array<{ rank: number; modelInvocable?: boolean; label: string }>>();
			...
			const winnerFor = (skill: SkillFile) => {
				const list = byName.get(skill.name);
				if (list === undefined) return undefined;
				let best = list[0];
				for (const cand of list) if (best === undefined || cand.rank < best.rank) best = cand;
				return best;
			};
			const mark = (skills: SkillFile[], ownRank: number) => {
		...
		async setStatus(body) {
			...
			const { roots, projectRoot } = await computeRoots(cwd, opts);
			if (projectRoot === null) throw new ApiError(400, '当前页没有会话工作区：按项目开关需要项目上下文');
			...
			if (isUserScope && state.globalDefaultOff) {
				...
				if (plan.existing === undefined) throw new ApiError(404, `skill 不存在：${name0}`);
		...
		async exportZip(body) {
			...
			const names = Array.isArray(body.names)
				? [...new Set(body.names.filter((n): n is string => typeof n === 'string' && NAME_RE.test(n)))]
				: [];
		...
		async validate(body) {
			...
			if (view.configCorrupt === true) issues.push({ code: 'config-corrupt', severity: 'error', message: '项目配置无法解析' });
			if (view.configFuture === true) issues.push({ code: 'config-future', severity: 'error', message: `项目配置版本高于 Host ${PROJECT_API_VERSION}` });
			for (const row of view.identities) {
				if (row.enabled !== row.modelInvocable) issues.push({ code: 'invocation-drift', severity: 'warning', name: row.name, message: `配置为${row.enabled ? '启用' : '停用'}，磁盘状态为${row.modelInvocable ? '可调用' : '不可调用'}` });
			}
			for (const [name, source] of Object.entries(config.sources || {})) {
				if (!identities.has(name)) issues.push({ code: 'orphan-registration', severity: 'warning', name, action: 'preserve', message: '来源登记没有对应项目文件；只读校验不会注销或删除' });
				if (source.generated === true && typeof source.copyHash !== 'string' && typeof source.contentHash !== 'string') issues.push({ code: 'managed-hash-missing', severity: 'error', name, action: 'preserve', message: '托管副本缺少可验证哈希，必须人工处理' });
				if (source.originType === 'github' && (![source.originRepository, source.originPath, source.originRef, source.originBundleHash].every((value) => typeof value === 'string' && value !== ''))) issues.push({ code: 'github-provenance-incomplete', severity: 'error', name, action: 'preserve', message: 'GitHub 来源信息不完整，禁止自动更新或清理' });
			}
			return {
				apiVersion: PROJECT_API_VERSION,
				projectRoot: view.projectRoot,
		...
		async ['presets.save'](body) {
			...
			const skills: SkillPreset['skills'] = {};
		...
		async ['presets.apply'](body) {
			...
			const { view, report } = await mutateProject(cwd, opts, async (ctx) => {
				...
				for (const [n, sel] of Object.entries(preset.skills)) {
					...
					if (identity === undefined) throw new ApiError(404, `skill 不存在：${n}`);
					await applySourceSelection(ctx.projectRoot, ctx.config, identity, sourceKey, deps, deps.logger, ctx.ledger);
				}
			});
	...
	return async (req, res) => {
		...
		try {
			...
			for await (const chunk of req) {
				bytes += chunk.length;
				if (bytes > 8 * 1024 * 1024) throw new ApiError(413, '请求体过大');
				chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk));
			}
			let body: ApiBody;
			try {
				const parsed: unknown = JSON.parse(Buffer.concat(chunks).toString('utf8') || '{}');
				body = parsed !== null && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as ApiBody : {};
			} catch {
			...
			if (value !== null && typeof value === 'object' && '__zip' in value && Buffer.isBuffer(value.__zip) && 'filename' in value && typeof value.filename === 'string') {
				const zipValue = value as ZipValue;
				res.writeHead(200, {
...
function summarizeIdentity(view: ProjectView, skillName: string): ProjectIdentityRow | null {
