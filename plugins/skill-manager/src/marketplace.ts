...
export function createMarketplace(options = {}) {
	...
	async function loadBundle(entry) {
		...
		if (manifest.name !== entry.name) throw new ApiError(409, `远程 Skill 名称与市场条目不一致：${manifest.name}`, 'MARKET_NAME_MISMATCH');
		return {
			files: output,
	...
	async function projectItemState(projectRoot, state, entry, meta) {
		...
		if (existing.hash !== selection.marketHash) return { status: 'modified', existing, selection };
