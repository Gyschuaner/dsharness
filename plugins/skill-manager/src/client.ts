...
(function () {
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			function GitHubMark(props: DynamicValue) {
				...
				return h(
					...
					activeDetail
						? h('aside', { className: 'sk-drawer sk-marketDrawer', role: 'dialog', 'aria-label': activeDetail.name + ' 详情' },
							h('div', { className: 'sk-drawerHead' }, h(MarketIcon, { src: activeDetail.iconUrl }), h('div', { className: 'sk-marketDrawerIdentity' }, h('div', { className: 'sk-drawerName' }, activeDetail.name), h('a', { className: 'sk-marketLink', href: activeDetail.url || ('https://github.com/' + activeDetail.repository), target: '_blank', rel: 'noreferrer' }, h(GitHubMark, { size: 13, className: 'sk-githubMark sk-githubMarkSmall' }), activeDetail.repository, h(P.IconRightUpOutline14))), h('button', { type: 'button', className: 'sk-icBtn', 'aria-label': '关闭详情', title: '关闭（Esc）', onClick: function () { setSelected(null); setDetail(null); } }, h(P.IconCloseOutline16))),
							...
							h('div', { className: 'sk-marketFoot' }, h('a', { className: 'sk-githubButton', href: activeDetail.url || ('https://github.com/' + activeDetail.repository), target: '_blank', rel: 'noreferrer', 'aria-label': '在 GitHub 中查看 ' + activeDetail.repository }, h(GitHubMark, { size: 18 }), h('span', null, '在 GitHub 中查看'), h('span', { className: 'sk-githubArrow' }, h(P.IconRightUpOutline14))), h(Button, { className: 'sk-marketInstallBtn', disabled: !canInstall || previewLoading || busy, onClick: openPreview }, previewLoading ? '检查中…' : detailStatus === 'update-available' ? '更新到当前项目' : detailStatus === 'installed' ? '重新安装' : canInstall ? '安装到当前项目' : '需要人工处理'))
						)
						: null,
