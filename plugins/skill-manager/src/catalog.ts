...
interface AgentPreset { id: string; path: string }
interface AgentPresetsService { list(): Promise<unknown[]> }
...
interface ResolvedCatalogOptions extends CatalogOptions { home: string }
...
export interface SkillRoot { id: string; scope: Exclude<SkillScope, 'bundled'>; label: string; dir: string; rank: number }
export interface BundledGroup { presetId: string; label: string; skills: SkillFile[] }
...
export interface SkillIdentity { name: string; sources: SkillSource[]; v1?: IdentityV1 }
...
export interface ProjectIdentityRow extends UnknownRecord {
	...
	whenToUse?: string;
	tags: string[];
	sources: Array<UnknownRecord & { key: string; modelInvocable: boolean; mtimeMs: number }>;
	defaultSourceKey: string | null;
	sourceKey: string | null;
	effectiveSourceKey: string | null;
	specialized: boolean;
...
function managedCopyMarker(entry: { copyHash?: string; contentHash?: string } | undefined): string | undefined {
	return entry?.copyHash ?? entry?.contentHash;
}

/** Precedence ranks mirroring dsh-skill-filesystem (lower wins). */
export const RANKS = { 'project-dsh': 100, 'project-agents': 200, global: 300, 'user-dsh': 400, 'user-agents': 500, bundled: 600 } as const;
...
const DSH_RESOLVER_ORDER: string[] = ['project-dsh', 'project-agents', 'global-codex', 'global-claude', 'user-dsh', 'user-agents'];
...
const PRODUCT_SOURCE_ORDER: string[] = ['project-dsh', 'project-agents', 'user-dsh', 'user-agents', 'global-codex', 'global-claude'];
function dshResolverIndex(key: string): number {
	const i = DSH_RESOLVER_ORDER.indexOf(key);
	return i === -1 ? DSH_RESOLVER_ORDER.length : i;
}
function productOrderIndex(key: string): number {
	const i = PRODUCT_SOURCE_ORDER.indexOf(key);
	return i === -1 ? PRODUCT_SOURCE_ORDER.length : i;
}

function optsOf(opts?: CatalogOptions): ResolvedCatalogOptions {
	const o: CatalogOptions = opts && typeof opts === 'object' ? { ...opts } : {};
	o.home = typeof o.home === 'string' && o.home.length > 0 ? resolve(o.home) : homedir();
	return o as ResolvedCatalogOptions;
}
...
async function renameWithRetry(from: string, to: string): Promise<void> {
	...
	for (let attempt = 0; attempt < 6; attempt += 1) {
		try {
			...
			if (code === undefined || !TRANSIENT_RENAME_CODES.has(code) || attempt === 5) throw error;
 ...
 */
export function parseSkill(raw: string): { name: string; description: string; whenToUse?: string; disableModelInvocation?: boolean; body: string } {
 ...
 */
export function patchInvocationFlag(raw: string, setTrue: boolean): { content: string; changed: boolean } {
	if (typeof raw !== 'string' || raw.length === 0) throw new ApiError(400, '内容为空');
...
export async function isShadowFile(path: string): Promise<boolean> {
	try {
...
export function markerContent(name: string, projectRoot: string): string {
	return [
		'---',
		`name: ${name}`,
...
export function shadowStubPath(projectRoot: string, name: string): string {
...
function legacyStubPath(projectRoot: string, name: string): string {
	return join(projectRoot, '.dsh', 'skills', `${name}.md`);
}

/** Atomic text rewrite with an optional transaction undo. */
async function writeTextWithLedger(path: string, content: string, ledger?: MutationLedger): Promise<void> {
 ...
 */
async function removePathWithLedger(path: string, ledger?: MutationLedger): Promise<boolean> {
...
function globalRoots(o: ResolvedCatalogOptions): Array<{ id: string; label: string; dir: string }> {
	return [
 ...
 */
export async function computeRoots(cwd?: string | null, opts?: CatalogOptions): Promise<{ roots: SkillRoot[]; projectRoot: string | null }> {
	const o = optsOf(opts);
 ...
 */
export async function discoverInRoot(dir: string): Promise<{ exists: boolean; skills: SkillFile[] }> {
	let entries: import('node:fs').Dirent[];
	try {
		entries = await readdir(dir, { withFileTypes: true });
	} catch (error) {
		if (errorCode(error) === 'ENOENT' || errorCode(error) === 'ENOTDIR') return { exists: false, skills: [] };
		throw error;
	}
	...
	for (const entry of sorted) {
		...
		const skill: SkillFile = { name: identity, title: identity, path, format, mtimeMs: 0, description: '', readOnly: false };
		try {
 ...
 */
export async function discoverBundled(agentPresets?: AgentPresetsService, _opts?: CatalogOptions): Promise<BundledGroup[]> {
	...
	for (const preset of presets) {
		if (preset === null || typeof preset !== 'object' || !('path' in preset) || !('id' in preset) || typeof preset.path !== 'string' || typeof preset.id !== 'string') continue;
		const validPreset = preset as unknown as AgentPreset;
...
export async function walkSkillFiles(dir: string): Promise<string[]> {
	const rootReal = await realpath(dir).catch(() => resolve(dir));
	const out: string[] = [];
	const seen = new Set<string>();
	async function rec(d: string, depth: number): Promise<void> {
		if (depth > 8) return;
		let entries;
		try {
...
function sourceRank(key: string): number {
	...
	return key in RANKS ? RANKS[key as keyof typeof RANKS] : RANKS.bundled;
}
/** Stable product-priority sort for source keys. */
function sourceKeysByPriority(keys: string[]): string[] {
 ...
 */
export async function hashSkillSourceLegacy(path: string, format: SkillFormat): Promise<string> {
	if (format === 'flat') {
		const raw = await readFile(path, 'utf8');
 ...
 */
export async function hashMatches(stored: unknown, path: string, format: SkillFormat, current: string): Promise<boolean> {
...
function copyLocation(projectRoot: string, name: string, format: SkillFormat): string {
 ...
 */
export async function copySkillToProject(projectRoot: string, name: string, sourcePlan: CopySourcePlan, flagSetTrue: boolean, opts?: CatalogOptions, ledger?: MutationLedger): Promise<string> {
	const o = optsOf(opts);
 ...
 */
export async function buildIdentityCatalog(cwd?: string | null, opts?: CatalogOptions, config?: ProjectConfig): Promise<{ identities: Map<string, SkillIdentity>; projectRoot: string | null; roots: SkillRoot[]; bundled: BundledGroup[] }> {
	const o = optsOf(opts);
	...
	const pushSource = (name: string, key: string, label: string, scope: SkillScope, rank: number, skill: SkillFile, extra: Partial<SkillSource>) => {
		...
		const source: SkillSource = Object.assign({
 ...
 */
function classifyIdentity(identity: SkillIdentity, projectConfig: ProjectConfig, _projectRoot: string | null): {
	mechanism: 'self' | 'copy' | 'original';
	defaultSourceKey: string | null;
	sourceKey: string | null;
	effectiveSource: SkillSource | null;
	specialized: boolean;
	winner: SkillSource | null;
} {
	...
	let winner: SkillSource | null = null;
...
export async function reconcileProject(
	...
	const stubPath = (name: string) => shadowStubPath(projectRoot, name);
	const legacyStub = (name: string) => legacyStubPath(projectRoot, name);
	const isStub = async (p: string): Promise<boolean> => {
		try {
	...
	if (projectRoot !== null && sweepOrphans) {
		for (const [name, entry] of Object.entries(projectConfig.sources)) {
			...
			if (copySrc === undefined) {
				...
				if (flatFile) copySrc = { path: flatP, format: 'flat' } as SkillSource;
				else if (dirFile) copySrc = { path: dirP, format: 'dir' } as SkillSource;
			}
			if (copySrc === undefined) {
 ...
 */
async function removeMarkerStub(projectRoot: string, name: string, report: ReconcileReport, ledger?: MutationLedger): Promise<void> {
 ...
 */
export async function ensureManagedCopy(
	projectRoot: string,
	projectConfig: ProjectConfig,
	identity: SkillIdentity,
	sourceKey: string,
	flagSetTrue: boolean,
	opts: CatalogOptions | undefined,
	report: ReconcileReport,
	recordSource = true,
	ledger?: MutationLedger,
): Promise<{ copyCreated: boolean; copyPath: string }> {
	const name = identity.name;
 ...
 *   broken selection).
 */
export async function applySourceSelection(
	projectRoot: string,
	projectConfig: ProjectConfig,
	identity: SkillIdentity,
	sourceKey: string | null,
	opts?: CatalogOptions,
	_logger?: CatalogOptions['logger'],
	ledger?: MutationLedger,
): Promise<{ changed: boolean; copyCreated: boolean; report: ReconcileReport }> {
	const report: ReconcileReport = { created: [], removed: [], rewritten: [], conflicts: [], failed: [] };
	const name = identity.name;
	const enabledSet = new Set(projectConfig.enabled);
	const wantEnabled = enabledSet.has(name);
	...
	let dshWinner: SkillSource | null = null;
	...
	if (prev && prev.generated === true) {
		...
		if (existingCopy !== undefined) {
			...
			} else throw new ApiError(409, `「${name}」的来源副本内容与登记不一致，不能切换为纯来源`);
		}
	}
