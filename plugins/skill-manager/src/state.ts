...
export interface SkillStateOptions {
	home?: string;
	faults?: {
		beforeProjectConfigWrite?(context: { path: string; config: UnknownRecord }): Promise<void> | void;
	};
}
...
function homeOf(opts?: SkillStateOptions): string {
...
export async function assertCwd(cwd: unknown): Promise<string | undefined> {
...
export function globalConfigPath(opts?: SkillStateOptions): string {
 ...
 */
export function withConfigLock<T>(key: string, task: () => Promise<T>): Promise<T> {
...
export function projectLockKey(projectRoot: string): string {
	return `project:${resolve(projectRoot)}`;
}
/** Lock key for the global config, its canonical file path. */
export function globalLockKey(opts?: SkillStateOptions): string {
 ...
 */
export function createLedger(): MutationLedger {
	const ops: Array<{ undo?: LedgerAction; cleanup?: LedgerAction }> = [];
	return {
		record(undo?: LedgerAction, cleanup?: LedgerAction) {
			if (undo || cleanup) ops.push({ ...(undo ? { undo } : {}), ...(cleanup ? { cleanup } : {}) });
		},
		async commit() {
 ...
 */
export async function readProjectConfig(projectRoot: string, _opts?: SkillStateOptions): Promise<ProjectConfigReadResult> {
...
export async function writeProjectConfig(
	...
	let onDisk: unknown = raw;
	if (onDisk === undefined) {
		try {
			onDisk = JSON.parse(await readFile(path, 'utf8'));
		} catch (error) {
			if (errorCode(error) === 'ENOENT' || errorCode(error) === 'ENOTDIR') onDisk = null;
	...
	const serialized: UnknownRecord = Object.assign({}, next);
...
export function emptyProjectConfig(projectRoot?: string): ProjectConfig {
	return {
...
export function normalizeProjectConfig(parsed: unknown, projectRoot: string): ProjectConfig {
...
export function migrateProjectConfig(parsed: unknown, projectRoot: string): ProjectConfig {
...
function normalizeGlobalConfig(parsed: unknown): GlobalConfig {
 ...
 */
export async function readGlobalConfig(opts?: SkillStateOptions): Promise<GlobalConfigReadResult> {
	...
	try {
		const rawObj: unknown = JSON.parse(await readFile(path, 'utf8'));
		if (!isRecord(rawObj)) {
...
export async function writeGlobalConfig(patch: UnknownRecord, opts?: SkillStateOptions): Promise<GlobalConfig> {
	const { config, raw, path, corrupt, future } = await readGlobalConfig(opts);
	if (corrupt) throw new ApiError(409, `全局 SKILL 配置损坏，拒绝覆盖：${path}`);
	if (future) throw new ApiError(409, `全局 SKILL 配置版本高于当前支持版本，拒绝降级覆盖：${path}`);
	const next: UnknownRecord = Object.assign({}, raw, config);
...
export function normalizeTagsMap(input: unknown): Record<string, string[]> {
 ...
 */
export function validateTagList(tags: unknown): string[] {
 ...
 */
export function normalizePresetsMap(input: unknown): Record<string, SkillPreset> {
	const out: Record<string, SkillPreset> = {};
	if (input === null || typeof input !== 'object' || Array.isArray(input)) return out;
	for (const [name, preset] of Object.entries(input)) {
		...
		const skills: Record<string, SkillPresetSelection> = {};
		...
		for (const [skillName, sel] of Object.entries(src)) {
			...
			const entry: SkillPresetSelection = {};
			if (isRecord(sel) && typeof sel.source === 'string' && sel.source.length > 0) entry.source = sel.source;
...
export function assertPresetName(name: unknown): string {
...
export function sha256Hex(data: string | NodeJS.TypedArray | DataView): string {
 ...
 */
export async function hashSkillSource(path: string, format: 'dir' | 'flat'): Promise<string> {
