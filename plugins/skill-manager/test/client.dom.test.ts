...
test('Skill market uses the shared page chrome and opens a safe repository detail drawer', async (t) => {
	...
	const marketRow = h.dom.window.document.querySelector('.sk-marketRow');
	assert.equal(marketRow.querySelector('.sk-marketMeta'), null, 'source metadata stays out of the market list');
	assert.ok(!marketRow.textContent.includes('acme/demo'), 'repository stays out of the market list');
	assert.ok(!marketRow.textContent.includes('精选'), 'source label stays out of the market list');
	assert.ok(!h.dom.window.document.body.textContent.includes('统一资源库'));
	...
	await h.click(marketRow);
	await h.flush();
	const marketDrawer = h.dom.window.document.querySelector('.sk-marketDrawer');
	assert.ok(marketDrawer);
	...
	assert.ok(marketDrawer.textContent.includes('来源：精选'), 'source label moves into the drawer');
	assert.ok(marketDrawer.textContent.includes('仓库：acme/demo'), 'repository moves into the drawer');
