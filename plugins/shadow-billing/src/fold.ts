...
export function readSessionLog(filePath: string, startOffset = 0): {
	...
	let header: FoldedHeader | null = null;
	...
	for (const frame of frames) {
		...
		for (const line of text.split('\n')) {
			...
			try {
				row = JSON.parse(line) as SessionEvent;
			} catch {
				continue;
			}
...
function readHeader(filePath: string): FoldedHeader | null {
	try {
		const buf = fs.readFileSync(filePath);
		...
		const row = JSON.parse(firstLine) as SessionEvent;
		if (row.type === 'session' && typeof row.id === 'string') {
...
/** 折叠一个会话日志文件。 */
export function foldSessionFile(
	store: Store,
	...
	let maxSeq = lastSeq;
	const usageByStep = new Map<string, StepUsage>();
	const stepKeyOf = (turn: number, step: number): string => `${turn}:${step}`;
	...
	for (const event of events) {
		...
		switch (event.type) {
			case 'request/header': {
				...
				const cfg = (headerData as { config?: { provider?: string; model?: string } } | undefined)?.config;
			...
			case 'request/context': {
				const d = event.data as { provider?: string; model?: string } | undefined;
			...
			case 'assistant/chunk': {
				...
				const chunk = d?.chunk;
				if (chunk?.type !== 'usage' || typeof d?.turn !== 'number' || typeof d?.step !== 'number') break;
...
/** 扫描全部会话日志并折叠。返回聚合结果。 */
export function foldAllSessions(
	store: Store,
	pricing: PricingConfig,
	sessionsRoot: string,
	logger?: Logger,
