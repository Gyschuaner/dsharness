...
(function () {
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			/** 统计卡：淡入上移入场。 */
			function StatCard(props: { label: string; value: string; hint?: string; accent?: boolean }): React.ReactNode {
				return h('div', { className: 'sb-card' + (props.accent ? ' sb-cardAccent' : ''), style: prefersReducedMotion() ? undefined : { animation: 'sb-card-in .2s ease-out both' } },
					h('div', { className: 'sb-cardLabel' }, props.label),
					h('div', { className: 'sb-cardValue' }, props.value),
					props.hint ? h('div', { className: 'sb-cardHint' }, props.hint) : null,
				);
			}

			/** 会话头部徽标：累计 token + 估算费用，点击弹详情层。 */
			function SessionBadge(props: Record<string, unknown>): React.ReactNode {
				const sessionId = typeof props.sessionId === 'string' ? props.sessionId : '';
				const [data, setData] = React.useState<SessionValue | null>(null);
				...
				React.useEffect(() => {
					...
					let alive = true;
					const load = (): void => {
						apiGet<SessionValue>(`/api/shadow-billing/session?sessionId=${encodeURIComponent(sessionId)}`).then((v) => {
							if (!alive) return;
							if (v === null) setFailed(true);
							else { setData(v); setFailed(false); }
						});
					};
					load();
				...
				return h('div', { className: 'sb-badgeWrap' },
					h('button', {
						type: 'button',
						...
						h('span', { className: 'sb-badgeCost' }, `≈¥${shownCost.toFixed(2)}`),
						h('span', { className: 'sb-badgeTokens' }, fmtTokens(tokens)),
					),
					open ? h('div', { className: 'sb-popover', role: 'dialog', 'data-testid': 'sb-popover' },
						h('div', { className: 'sb-popoverTitle' }, '本会话估算消耗'),
						data === null ? h('p', { className: 'sb-muted' }, '暂无记录') : h(React.Fragment, null,
							h('div', { className: 'sb-popoverRow' }, h('span', null, '请求'), h('b', null, String(data.requests))),
							h('div', { className: 'sb-popoverRow' }, h('span', null, '未命中输入'), h('b', null, fmtTokens(data.inputTokens))),
							h('div', { className: 'sb-popoverRow' }, h('span', null, '缓存命中'), h('b', null, fmtTokens(data.cacheReadTokens))),
							h('div', { className: 'sb-popoverRow' }, h('span', null, '输出'), h('b', null, fmtTokens(data.outputTokens))),
							h('div', { className: 'sb-popoverRow sb-popoverRowTotal' }, h('span', null, '估算费用'), h('b', null, `¥${fmtCost(data.costNano)}`)),
							data.lastAt !== null ? h('p', { className: 'sb-muted' }, `最近调用 ${fmtTime(data.lastAt)}`) : null,
						),
						h('p', { className: 'sb-footnote' }, '按 DeepSeek Flash 峰谷价影子计费，非真实账单'),
					) : null,
				);
			}

			/** 趋势面积图：SVG 路径绘制入场 + 数据点。 */
			function TrendChart(props: { daily: DailyRow[] }): React.ReactNode {
				...
				return h('div', { className: 'sb-trend' },
					h('svg', {
						viewBox: `0 0 ${W} ${H}`,
						...
						h('defs', null,
							h('linearGradient', { id: 'sb-trend-fill', x1: '0', y1: '0', x2: '0', y2: '1' },
								h('stop', { offset: '0%', stopColor: 'var(--dsw-alias-brand-primary)', stopOpacity: 0.35 }),
								h('stop', { offset: '100%', stopColor: 'var(--dsw-alias-brand-primary)', stopOpacity: 0.02 }),
							),
						),
						h('path', { d: area, fill: 'url(#sb-trend-fill)', className: 'sb-trendArea' }),
						h('path', {
							d: line,
							fill: 'none',
			...
			/** 用量页签主视图。 */
			function UsageView(): React.ReactNode {
				const [days, setDays] = React.useState(7);
				const [summary, setSummary] = React.useState<SummaryValue | null>(null);
				const [models, setModels] = React.useState<ModelRow[]>([]);
				const [daily, setDaily] = React.useState<DailyRow[]>([]);
				const [reqs, setReqs] = React.useState<RequestsValue | null>(null);
				const [page, setPage] = React.useState(0);
				const [refreshing, setRefreshing] = React.useState(false);

				const load = React.useCallback((d: number, p: number): void => {
					apiGet<SummaryValue>(`/api/shadow-billing/summary?days=${d}`).then(setSummary);
					apiGet<{ models: ModelRow[] }>(`/api/shadow-billing/by-model?days=${d}`).then((v) => setModels(v?.models ?? []));
					apiGet<{ daily: DailyRow[] }>(`/api/shadow-billing/daily?days=${d}`).then((v) => setDaily(v?.daily ?? []));
					apiGet<RequestsValue>(`/api/shadow-billing/requests?days=${d}&page=${p}&size=10`).then((v) => {
						if (v !== null) { setReqs(v); setPage(p); }
					});
				}, []);
				...
				const daysOptions: Array<[number, string]> = [[1, '今天'], [7, '7 天'], [30, '30 天'], [90, '90 天'], [0, '全部']];

				return h('div', { className: 'sb-root', 'data-testid': 'sb-usage-view' },
					h('div', { className: 'sb-head' },
						h('h2', null, '用量'),
						h('div', { className: 'sb-headRight' },
							h('div', { className: 'sb-days', role: 'tablist' },
								daysOptions.map(([d, label]) => h('button', {
									key: d,
									type: 'button',
									role: 'tab',
							...
							h('button', { type: 'button', className: 'sb-btn', onClick: refresh, disabled: refreshing, title: '重新折叠日志并刷新' },
								refreshing ? '刷新中…' : '刷新'),
						),
					),
					h('div', { className: 'sb-cards' },
						h(StatCard, { label: '估算费用', value: `¥${shownCost.toFixed(2)}`, hint: '影子计费 · 非真实账单', accent: true }),
						h(StatCard, { label: 'Token 消耗', value: fmtTokens(totalTokens), hint: `${fmtTokens(summary?.inputTokens ?? 0)} 未命中 + ${fmtTokens(summary?.cacheReadTokens ?? 0)} 命中 + ${fmtTokens(summary?.outputTokens ?? 0)} 输出` }),
						h(StatCard, { label: '请求次数', value: String(summary?.requests ?? 0), hint: `${days === 0 ? '全部' : '近 ' + days + ' 天'}` }),
						h(StatCard, { label: '缓存命中率', value: `${cacheRate.toFixed(1)}%`, hint: '缓存命中 token 占比' }),
					),
					h(TrendChart, { daily }),
					h('div', { className: 'sb-grid' },
						h('section', { className: 'sb-panel' },
							h('h3', null, '模型排行'),
							models.length === 0 ? h('p', { className: 'sb-muted' }, '暂无记录') : h('table', { className: 'sb-table' },
								h('thead', null, h('tr', null,
									h('th', null, '模型'),
									h('th', { className: 'sb-num' }, '请求'),
									h('th', { className: 'sb-num' }, 'Token'),
									h('th', { className: 'sb-num' }, '估算费用'),
								)),
								h('tbody', null, models.map((m, i) => h('tr', { key: i },
									h('td', null, m.model),
									h('td', { className: 'sb-num' }, String(m.requests)),
									h('td', { className: 'sb-num' }, fmtTokens(m.inputTokens + m.outputTokens + m.cacheReadTokens)),
									h('td', { className: 'sb-num' }, `¥${fmtCost(m.costNano)}`),
								))),
							),
						),
						h('section', { className: 'sb-panel' },
							h('h3', null, '请求明细'),
							reqs === null || reqs.rows.length === 0 ? h('p', { className: 'sb-muted' }, '暂无记录') : h(React.Fragment, null,
								h('table', { className: 'sb-table sb-tableDense' },
									h('thead', null, h('tr', null,
										h('th', null, '时间'),
										h('th', null, '模型'),
										h('th', { className: 'sb-num' }, '输入'),
										h('th', { className: 'sb-num' }, '命中'),
										h('th', { className: 'sb-num' }, '输出'),
										h('th', { className: 'sb-num' }, '费用'),
									)),
									h('tbody', null, reqs.rows.map((r) => h('tr', { key: r.record_id },
										h('td', { className: 'sb-mono' }, fmtTime(r.created_at)),
										h('td', null, r.model),
										h('td', { className: 'sb-num' }, fmtTokens(r.input_tokens)),
										h('td', { className: 'sb-num' }, fmtTokens(r.cache_read_tokens)),
										h('td', { className: 'sb-num' }, fmtTokens(r.output_tokens)),
										h('td', { className: 'sb-num' }, `¥${fmtCost(r.cost_nano)}`),
									))),
								),
								h('div', { className: 'sb-pager' },
									h('button', { type: 'button', className: 'sb-btn', disabled: page <= 0, onClick: () => load(days, page - 1) }, '上一页'),
									h('span', { className: 'sb-muted' }, `第 ${page + 1} 页 · 共 ${reqs.total} 条`),
									h('button', { type: 'button', className: 'sb-btn', disabled: (page + 1) * 10 >= reqs.total, onClick: () => load(days, page + 1) }, '下一页'),
								),
							),
						),
					),
					h('p', { className: 'sb-footnote' },
						'费用按 DeepSeek Flash 官方峰谷价（¥/1M：命中 0.05 / 未命中 1.5 / 输出 4.5，高峰 ×2）影子计费，仅供成本核算参考，非真实账单。'),
				);
			}
			...
			function SettingsView(): React.ReactNode {
				...
				return h('div', { className: 'sb-settings', 'data-testid': 'sb-settings' },
					h('h2', null, '用量计费'),
					h('p', null, '按 DeepSeek Flash 官方峰谷价对本地模型调用做影子计费。数据来自 DSH 会话日志中的真实 token 用量，仅作成本核算参考，不是真实账单。'),
					h('h3', null, '价目表（¥ / 1M tokens）'),
					h('table', { className: 'sb-table' },
						h('thead', null, h('tr', null, h('th', null, '档位'), h('th', null, '低谷价'), h('th', null, '高峰价（×2）'))),
						h('tbody', null,
							h('tr', null, h('td', null, '缓存命中'), h('td', null, '0.05'), h('td', null, '0.10')),
							h('tr', null, h('td', null, '未命中'), h('td', null, '1.5'), h('td', null, '3.0')),
							h('tr', null, h('td', null, '输出'), h('td', null, '4.5'), h('td', null, '9.0')),
						),
					),
					h('p', { className: 'sb-muted' },
						'高峰时段：北京时间 9:00-12:00 / 14:00-18:00；2026-08-23 起周末全天低谷价。峰谷按北京时间判定，不随用户时区漂移。'),
					h('h3', null, '数据源'),
					status === null ? h('p', { className: 'sb-muted' }, '读取中…') : h(React.Fragment, null,
						h('p', { className: 'sb-mono' }, status.sessionsRoot),
						h('p', { className: 'sb-muted' },
