...
export function isPeakHour(ts: number, weekendOffpeakSince: number): boolean {
	const weekday = beijingWeekday(ts);
	if (ts >= weekendOffpeakSince && (weekday === 0 || weekday === 6)) return false;
	return PEAK_HOURS.has(beijingHour(ts));
}

/** 归一模型名：别名映射（大小写不敏感回退）。 */
export function resolveModel(raw: string, config: PricingConfig): string {
...
export function resolvePricing(
	userModels?: Record<string, Partial<PriceEntry>>,
	...
	if (userAliases !== undefined) Object.assign(aliases, userAliases);
	return {
		models,
		aliases,
