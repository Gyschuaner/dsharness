	...
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			style.textContent = [
				...
				'.pm-switch{appearance:none;position:relative;width:36px;height:20px;border:0;border-radius:999px;background:var(--dsw-alias-fill-tsp-secondary);cursor:pointer;padding:0;transition:background .14s ease}',
				'.pm-switch:after{content:"";position:absolute;width:16px;height:16px;left:2px;top:2px;border-radius:50%;background:var(--dsw-alias-bg-base);box-shadow:0 1px 3px rgba(0,0,0,.18);transition:transform .14s ease}',
				'.pm-switchOn{background:var(--dsw-alias-brand-primary)}',
				'.pm-switchOn:after{transform:translateX(16px)}',
			...
			function LocalDrawer(props: LocalDrawerProps): React.ReactNode {
				...
				return h('aside', { className: 'pm-drawer', role: 'dialog', 'aria-label': plugin.name + ' 详情' },
					...
					h('div', { className: 'pm-drawerBody' },
						...
						h('section', { className: 'pm-section' }, h('h4', { className: 'pm-sectionTitle' }, '挂载与生效'), h(KeyValues, { rows: [['期望状态', plugin.enabled ? '已启用' : '已停用'], ['当前运行', plugin.runtimePhase || (plugin.runtimeEnabled === false ? '未挂载' : '—')], ['组合行', plugin.rowId], ['变更生效', '重启 Web 后']] })),
				...
				h('footer', { className: 'pm-drawerFoot' },
					...
					h(Button, { type: 'button', primary: true, disabled: props.busy || plugin.protected, onClick: function () { props.onToggle(!plugin.enabled); } }, plugin.protected ? '运行所必需' : plugin.enabled ? '停用插件' : '启用插件')
				)
				);
			}
			...
			function PluginManagerSection(props: { api: PluginApi }): React.ReactNode {
				...
				function importPlugin(source: string): Promise<unknown> {
					...
					return api.call('import', { source: source }).then(function (result) {
						...
						return Promise.all([loadLocal(), loadMarket()]);
				...
				function installMarket() {
					...
					api.call('marketplace.install', { id: target.id }).then(function (result) {
						...
						return Promise.all([loadLocal(), loadMarket(), api.call('marketplace.detail', { id: target.id, force: true }).then(function (value) { setMarketDetail(value); })]);
					}).catch(function (reason) { notify(reason instanceof Error ? reason.message : String(reason), true); }).finally(function () { setBusy(''); });
				}
				...
				if (loading) body = h(PluginLoadingState);
					...
					return h('div', { key: plugin.name, className: 'pm-row pm-rowClick' + (selectedLocal && selectedLocal.name === plugin.name ? ' pm-rowSelected' : ''), onClick: function () { setSelectedMarket(null); setSelectedLocal(plugin); } },
						...
						h('div', { className: 'pm-rowSide' }, h(Switch, { checked: plugin.enabled, disabled: busy === plugin.name || plugin.protected, label: (plugin.enabled ? '停用 ' : '启用 ') + plugin.name, title: plugin.protected ? '此插件维持当前管理页面运行' : undefined, onChange: function (enabled) { toggle(plugin, enabled); } }))
					);
				}));
