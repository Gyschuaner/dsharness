...
interface PluginLocalView {
	...
	manifest: { hostEntry: string | null; clientEntry: string | null; bundlePatch: string | null };
}
...
interface PluginClientSlots {
	register(config: Record<string, unknown>, component: React.ComponentType<{ api: PluginApi }>): unknown;
...
(function () {
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			var existingStyle = document.querySelector<HTMLStyleElement>('style[data-plugin="dsh-plugin-manager"]');
			...
			style.textContent = [
				...
				'@media(max-width:680px){.pm-root{margin:0}.pm-head{padding-left:0}.pm-tabs,.pm-toolbar,.pm-list{padding-left:0;padding-right:0}.pm-toolbar{flex-wrap:wrap}.pm-search{flex-basis:100%}.pm-drawer{top:61px;width:calc(100vw - 12px);max-width:none}.pm-row{padding-left:6px;padding-right:6px}}'
			].join('');
			if (!existingStyle) document.head.appendChild(style);

			function Icon(props: { type: string; size?: number }): React.ReactNode {
				var type = props.type;
				var size = props.size || 16;
				var common = { width: size, height: size, viewBox: '0 0 16 16', fill: 'none', 'aria-hidden': true };
				if (type === 'search') return h('svg', common, h('circle', { cx: 7, cy: 7, r: 4.5, stroke: 'currentColor', strokeWidth: 1.4 }), h('path', { d: 'M10.4 10.4 14 14', stroke: 'currentColor', strokeWidth: 1.4, strokeLinecap: 'round' }));
				if (type === 'external') return h('svg', common, h('path', { d: 'M9 2h5v5M8 8l6-6M13 9v3.5A1.5 1.5 0 0 1 11.5 14h-8A1.5 1.5 0 0 1 2 12.5v-8A1.5 1.5 0 0 1 3.5 3H7', stroke: 'currentColor', strokeWidth: 1.3, strokeLinecap: 'round', strokeLinejoin: 'round' }));
				if (type === 'plus') return h('svg', common, h('path', { d: 'M8 3v10M3 8h10', stroke: 'currentColor', strokeWidth: 1.4, strokeLinecap: 'round' }));
			...
			function Button(props: PluginButtonProps): React.ReactNode {
				var next: PluginButtonProps = Object.assign({}, props);
			...
			function Switch(props: PluginSwitchProps): React.ReactNode {
				return h('button', {
					type: 'button',
					role: 'switch',
					...
					title: props.title,
					onClick: function (event: React.MouseEvent<HTMLButtonElement>) { event.stopPropagation(); props.onChange(!props.checked); }
				});
			}

			function Search(props: PluginSearchProps): React.ReactNode {
			...
			function ImportDialog(props: ImportDialogProps): React.ReactNode {
				var [source, setSource] = React.useState('');
				var [busy, setBusy] = React.useState(false);
				var [error, setError] = React.useState('');
				function submit() {
					if (source.trim() === '') { setError('请输入插件来源'); return; }
					setBusy(true); setError('');
					props.onSubmit(source.trim()).then(props.onClose, function (reason) {
						setError(reason instanceof Error ? reason.message : String(reason));
					}).finally(function () { setBusy(false); });
				}
				return h('div', { className: 'pm-dialogBackdrop', onMouseDown: function (event: React.MouseEvent<HTMLDivElement>) { if (event.target === event.currentTarget && !busy) props.onClose(); } },
					h('div', { className: 'pm-dialog', role: 'dialog', 'aria-modal': true, 'aria-label': '导入插件' },
						h('h3', null, '导入插件'),
						h('p', null, '支持 npm 包、GitHub 仓库或本地绝对目录。安装时默认不执行第三方安装脚本。'),
						h('div', { className: 'pm-field' },
							h('label', { htmlFor: 'pm-import-source' }, '插件来源'),
							h('input', { id: 'pm-import-source', className: 'pm-input', autoFocus: true, value: source, disabled: busy, placeholder: '例如 dsh-example@latest 或 github:user/repo', onChange: function (event: React.ChangeEvent<HTMLInputElement>) { setSource(event.target.value); }, onKeyDown: function (event: React.KeyboardEvent<HTMLInputElement>) { if (event.key === 'Enter' && !busy) submit(); } })
						),
						error ? h('p', { className: 'pm-error', role: 'alert' }, error) : null,
						h('div', { className: 'pm-dialogActions' }, h(Button, { type: 'button', disabled: busy, onClick: props.onClose }, '取消'), h(Button, { type: 'button', primary: true, disabled: busy, onClick: submit }, busy ? '正在导入…' : '导入'))
					)
				);
			}

			function KeyValues(props: { rows: Array<[React.ReactNode, React.ReactNode]> }): React.ReactNode {
			...
			function Disclosure(props: { title: string; children?: React.ReactNode }): React.ReactNode {
				var [open, setOpen] = React.useState(false);
				return h(React.Fragment, null,
					h('button', { type: 'button', className: 'pm-disclosure', 'aria-expanded': open, onClick: function () { setOpen(!open); } }, props.title, h(open ? P.IconChevronUpOutline14 : P.IconChevronDownOutline14)),
					open ? h('div', { className: 'pm-disclosureBody' }, props.children) : null
				);
			}

			function LocalDrawer(props: LocalDrawerProps): React.ReactNode {
				var plugin = props.plugin;
				var readOnlyReason = !plugin.managed ? '此插件由系统组合管理，只读展示' : plugin.protected ? '此插件维持当前管理页面运行' : undefined;
				return h('aside', { className: 'pm-drawer', role: 'dialog', 'aria-label': plugin.name + ' 详情' },
					h('header', { className: 'pm-drawerHead' },
						h('div', { className: 'pm-drawerTitleRow' }, h('h3', { className: 'pm-drawerTitle' }, plugin.name), h('button', { type: 'button', className: 'pm-close', 'aria-label': '关闭详情', onClick: props.onClose }, h(P.IconCloseOutline16))),
						plugin.repository ? h('a', { className: 'pm-repo', href: 'https://github.com/' + plugin.repository, target: '_blank', rel: 'noreferrer' }, h(Icon, { type: 'github' }), 'github.com/' + plugin.repository, h(Icon, { type: 'external', size: 13 })) : null,
						h('p', { className: 'pm-drawerDesc' }, plugin.description || '此插件没有提供描述。')
					),
					h('div', { className: 'pm-drawerBody' },
						h('section', { className: 'pm-section' }, h('h4', { className: 'pm-sectionTitle' }, '版本与来源'), h(KeyValues, { rows: [['版本', plugin.version], ['来源', plugin.source], ['安装声明', plugin.spec]] })),
						h('section', { className: 'pm-section' }, h('h4', { className: 'pm-sectionTitle' }, '挂载与生效'), h(KeyValues, { rows: [['期望状态', plugin.enabled ? '已启用' : '已停用'], ['当前运行', plugin.runtimePhase || (plugin.runtimeEnabled === false ? '未挂载' : '—')], ['组合行', plugin.rowId], ['管理方式', plugin.managed ? 'Profile 覆盖；重启 Web 后生效' : '系统 Bundle；当前页面只读']] })),
						h(Disclosure, { title: '技术信息' }, h(KeyValues, { rows: [['Host 入口', plugin.manifest.hostEntry], ['Client 入口', plugin.manifest.clientEntry], ['Bundle patch', plugin.manifest.bundlePatch], ['许可证', plugin.license]] }))
				),
				h('footer', { className: 'pm-drawerFoot' },
					plugin.repository ? h('a', { className: 'pm-btn', href: 'https://github.com/' + plugin.repository, target: '_blank', rel: 'noreferrer' }, '在 GitHub 查看', h(Icon, { type: 'external', size: 13 })) : h(Button, { type: 'button', disabled: true }, '本地来源'),
				h(Button, { type: 'button', primary: true, disabled: props.busy || plugin.protected || !plugin.managed, title: readOnlyReason, onClick: function () { props.onToggle(!plugin.enabled); } }, !plugin.managed ? '系统组合只读' : plugin.protected ? '运行所必需' : plugin.enabled ? '停用插件' : '启用插件')
				)
				);
			}
				...
				function MarketDrawer(props: PluginMarketDrawerProps): React.ReactNode {
					...
					var registryStatusLabel = props.registry.status === 'fresh' ? 'Registry 已更新' : props.registry.status === 'stale' ? 'Registry 使用缓存' : 'Registry 暂不可用，当前保留精选';
					return h('aside', { className: 'pm-drawer', role: 'dialog', 'aria-label': item.repository + ' 详情' },
						...
						h('div', { className: 'pm-drawerBody' },
							h('section', { className: 'pm-section' }, h('h4', { className: 'pm-sectionTitle' }, '发现与安装'), h('p', { className: 'pm-restart' }, '自动搜索 npm 与 DSH Registry；只有含有效 dsh manifest 的精确版本可以一键安装。'), h('p', { className: 'pm-registryState' + (props.registry.status === 'fresh' ? '' : ' pm-registryStateWarning'), role: 'status' }, registryStatusLabel)),
			...
			function PluginLoadingState(): React.ReactNode {
				...
				var [selectedLocal, setSelectedLocal] = React.useState<PluginLocalView | null>(null);
				var [selectedMarket, setSelectedMarket] = React.useState<PluginMarketItem | null>(null);
				var [marketDetail, setMarketDetail] = React.useState<PluginMarketDetail | null>(null);
				...
				function loadMarket(search = query, cursor = '', append = false, force = false, sort = marketSort): Promise<void> {
					...
					}).finally(function () { if (request === marketRequest.current) setMarketLoading(false); });
				}
				...
				React.useEffect(function () {
					function onKey(event: KeyboardEvent): void {
						if (event.key !== 'Escape' || importOpen) return;
						if (selectedLocal) setSelectedLocal(null);
				...
				function toggle(plugin: PluginLocalView, enabled: boolean): Promise<void> {
					setBusy(plugin.name);
				...
				function importPlugin(source: string): Promise<unknown> {
					setBusy('import');
				...
				function openMarket(item: PluginMarketItem): void {
					setSelectedLocal(null); setSelectedMarket(item); setMarketDetail(null); setDetailLoading(true);
				...
				if (loading) body = h(PluginLoadingState);
					...
					return h('div', { key: plugin.name, className: 'pm-row pm-rowClick' + (selectedLocal && selectedLocal.name === plugin.name ? ' pm-rowSelected' : ''), onClick: function () { setSelectedMarket(null); setSelectedLocal(plugin); } },
						h('div', { className: 'pm-rowMain' }, h('span', { className: 'pm-pluginIcon' }, h(Icon, { type: 'plugin', size: 15 })), h('div', { className: 'pm-rowCopy' }, h('div', { className: 'pm-rowTitle' }, plugin.name), h('div', { className: 'pm-rowDesc' }, plugin.description || '此插件没有提供描述。'), h('div', { className: 'pm-rowMeta' }, h('span', null, plugin.version), h('span', null, '·'), h('span', null, plugin.source), !plugin.managed ? h('span', null, '· 只读') : plugin.protected ? h('span', null, '· 运行所必需') : null))),
						h('div', { className: 'pm-rowSide' }, h(Switch, { checked: plugin.enabled, disabled: busy === plugin.name || plugin.protected || !plugin.managed, label: (plugin.enabled ? '停用 ' : '启用 ') + plugin.name, title: !plugin.managed ? '此插件由系统组合管理，只读展示' : plugin.protected ? '此插件维持当前管理页面运行' : undefined, onChange: function (enabled) { toggle(plugin, enabled); } }))
					);
				}));
				else if (marketLoading && !marketLoaded) body = h('p', { className: 'pm-empty', role: 'status' }, '正在搜索 DSH Plugin Registry…');
				else body = h(React.Fragment, null, visibleMarket.length === 0 ? h('p', { className: 'pm-empty' }, '远程 Registry 中没有匹配且通过 manifest 校验的插件。') : h('div', { className: 'pm-list', 'data-testid': 'market-list' }, visibleMarket.map(function (item) {
					...
					return h('button', { key: item.id, type: 'button', className: 'pm-row pm-rowClick' + (selectedMarket && selectedMarket.id === item.id ? ' pm-rowSelected' : ''), onClick: function () { openMarket(item); } },
						h('div', { className: 'pm-marketMain' }, h(RemoteIcon, { src: item.iconUrl }), h('div', { className: 'pm-marketCopy' }, h('div', { className: 'pm-rowTitle' }, item.repository), h('div', { className: 'pm-rowDesc' }, item.description), h('div', { className: 'pm-rowMeta' }, item.marketSource === 'npm' ? 'npm Registry' : item.marketSource === 'registry' ? 'DSH Registry' : '精选', item.latestVersion ? ' · ' + item.latestVersion : ''))),
				...
				})), marketNextCursor ? h('div', { className: 'pm-loadMore' }, h(Button, { type: 'button', disabled: marketLoading, onClick: function () { if (marketNextCursor) loadMarket(query, marketNextCursor, true, false); } }, marketLoading ? '加载中…' : '加载更多')) : null);
				...
				return h('section', { className: 'pm-root', 'aria-label': 'Plugin Manager' },
					...
					selectedMarket ? h(MarketDrawer, { item: selectedMarket, detail: marketDetail, registry: registryInfo, loading: detailLoading, busy: busy === selectedMarket.id, onClose: function () { setSelectedMarket(null); }, onInstall: installMarket }) : null,
					importOpen ? h(ImportDialog, { onClose: function () { if (busy !== 'import') setImportOpen(false); }, onSubmit: importPlugin }) : null,
					toast ? h('div', { className: 'pm-toast' + (toast.error ? ' pm-toastError' : ''), role: 'status' }, toast.message) : null
				);
			}
			...
			module.exports.apply = function (ctx) {
				var slots = ctx.get('slots') as PluginClientSlots | undefined;
