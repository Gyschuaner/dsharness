...
function optionalPackageName(value: unknown): string | null {
	...
	if (!PACKAGE_NAME_RE.test(name)) throw new Error('packageName 不合法');
	return name;
}

function normalizeItem(value: unknown, index: number): RegistryItem {
	...
	if (!REPOSITORY_RE.test(repository)) throw new Error(`items[${index}].repository 不合法`);
	const id = cleanText(value.id ?? repository, `items[${index}].id`, 200);
	if (id !== repository) throw new Error(`items[${index}].id 必须等于 repository`);
 ...
 */
export function normalizeRegistry(value: unknown): PluginRegistry {
	if (!isRecord(value)) throw new Error('Registry 根节点必须是对象');
	if (value.schemaVersion !== REGISTRY_SCHEMA_VERSION) throw new Error(`Registry schemaVersion 必须是 ${REGISTRY_SCHEMA_VERSION}`);
	const generatedAt = cleanText(value.generatedAt, 'generatedAt', 80);
	if (Number.isNaN(Date.parse(generatedAt))) throw new Error('generatedAt 不是合法时间');
	if (!Array.isArray(value.items) || value.items.length > MAX_ITEMS) throw new Error(`items 必须是最多 ${MAX_ITEMS} 项的数组`);
