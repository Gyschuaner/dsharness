	...
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			function apiCall(op, payload) {
				return fetch('/api/plugin-manager', {
					...
					return response.json().then(function (data) {
						if (!response.ok || !data || data.ok !== true) throw new Error((data && data.error && data.error.message) || ('HTTP ' + response.status));
						return data.value;
					});
				});
			}
			...
			function ImportDialog(props) {
				...
				return h('div', { className: 'pm-dialogBackdrop', onMouseDown: function (event) { if (event.target === event.currentTarget && !busy) props.onClose(); } },
					h('div', { className: 'pm-dialog', role: 'dialog', 'aria-modal': true, 'aria-label': '导入插件' },
						...
						h('div', { className: 'pm-field' },
							...
							h('input', { id: 'pm-import-source', className: 'pm-input', autoFocus: true, value: source, disabled: busy, placeholder: '例如 dsh-example@latest 或 github:user/repo', onChange: function (event) { setSource(event.target.value); }, onKeyDown: function (event) { if (event.key === 'Enter' && !busy) submit(); } })
						),
			...
			function MarketDrawer(props) {
				...
				h('div', { className: 'pm-drawerBody' },
					loading ? h('p', { className: 'pm-loading' }, '正在读取 GitHub 信息…') : detail && detail.error ? h('p', { className: 'pm-error' }, detail.error) : h(React.Fragment, null,
				...
				h('footer', { className: 'pm-drawerFoot' },
					...
					h(Button, { type: 'button', primary: true, disabled: props.busy || loading || (status === 'installed' && !latest) || (detail && detail.manifest && detail.manifest.valid === false), onClick: props.onInstall }, props.busy ? '正在处理…' : actionText)
				)
				);
			}
			...
			function PluginManagerSection(props) {
				...
				function installMarket() {
					...
					setBusy(selectedMarket.id);
					api.call('marketplace.install', { id: selectedMarket.id }).then(function (result) {
						notify('已安装 ' + (result.plugin ? result.plugin.name : selectedMarket.repository) + '；重启 Web 后生效', false);
						return Promise.all([loadLocal(), loadMarket(), api.call('marketplace.detail', { id: selectedMarket.id, force: true }).then(setMarketDetail)]);
					}).catch(function (reason) { notify(reason instanceof Error ? reason.message : String(reason), true); }).finally(function () { setBusy(''); });
				}
				...
				return h('section', { className: 'pm-root', 'aria-label': 'Plugin Manager' },
					...
					body,
					selectedLocal ? h(LocalDrawer, { plugin: selectedLocal, busy: busy === selectedLocal.name, onClose: function () { setSelectedLocal(null); }, onToggle: function (enabled) { toggle(selectedLocal, enabled); } }) : null,
