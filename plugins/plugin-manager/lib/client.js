    ...
    window.__ModuleLoader__.load({
        ...
        factory: function (require) {
            ...
            function PluginManagerSection(props) {
                ...
                var [marketSource, setMarketSource] = React.useState('featured');
                ...
                if (loading)
                    ...
                    body = visibleMarket.length === 0 ? h('p', { className: 'pm-empty' }, '没有匹配的市场插件。') : h('div', { className: 'pm-list', 'data-testid': 'market-list' }, visibleMarket.map(function (item) {
                        ...
                        return h('button', { key: item.id, type: 'button', className: 'pm-row pm-rowClick' + (selectedMarket && selectedMarket.id === item.id ? ' pm-rowSelected' : ''), onClick: function () { openMarket(item); } }, h('div', { className: 'pm-marketMain' }, h(RemoteIcon, { src: item.iconUrl }), h('div', { className: 'pm-marketCopy' }, h('div', { className: 'pm-rowTitle' }, item.repository), h('div', { className: 'pm-rowDesc' }, item.description))), h('div', { className: 'pm-rowSide' }, h('span', { className: 'pm-status' + (item.status === 'update-available' ? ' pm-statusUpdate' : '') }, statusLabel), h(P.IconChevronRightOutline14)));
                    }));
                return h('section', { className: 'pm-root', 'aria-label': 'Plugin Manager' }, h('header', { className: 'pm-head' }, h('h2', null, 'Plugin'), h('span', { className: 'pm-context' }, 'Web 配置')), h('div', { className: 'pm-tabs', role: 'tablist' }, h('button', { type: 'button', role: 'tab', className: 'pm-tab' + (tab === 'local' ? ' pm-tabOn' : ''), 'aria-selected': tab === 'local', onClick: function () { setTab('local'); setQuery(''); setSelectedMarket(null); } }, '本地插件'), h('button', { type: 'button', role: 'tab', className: 'pm-tab' + (tab === 'market' ? ' pm-tabOn' : ''), 'aria-selected': tab === 'market', onClick: function () { setTab('market'); setQuery(''); setSelectedLocal(null); } }, '插件市场')), tab === 'market' ? h('div', { className: 'pm-marketFilters', role: 'tablist', 'data-testid': 'market-channels' }, h('button', { type: 'button', role: 'tab', className: 'pm-marketFilter' + (marketSource === 'featured' ? ' pm-marketFilterOn' : ''), 'aria-selected': marketSource === 'featured', onClick: function () { setMarketSource('featured'); setQuery(''); setSelectedMarket(null); } }, '精选'), h('button', { type: 'button', role: 'tab', className: 'pm-marketFilter' + (marketSource === 'registry' ? ' pm-marketFilterOn' : ''), 'aria-selected': marketSource === 'registry', onClick: function () { setMarketSource('registry'); setQuery(''); setSelectedMarket(null); } }, '发现')) : null, h('div', { className: 'pm-toolbar' }, h(Search, { value: query, onChange: setQuery, placeholder: tab === 'local' ? '搜索本地插件' : '搜索插件或 GitHub 仓库' }), tab === 'local' ? h(React.Fragment, null, h(Button, { type: 'button', onClick: function () { setImportOpen(true); } }, h(Icon, { type: 'plus', size: 14 }), '导入插件'), h(Button, { type: 'button', className: 'pm-iconBtn', title: '更多操作', 'aria-label': '更多操作' }, h(Icon, { type: 'dots' }))) : null), tab === 'market' ? h('p', { className: 'pm-helper' }, '精选自 GitHub · Registry 只读发现 · 安装前校验 DSH 插件清单') : null, tab === 'market' ? h('p', { className: 'pm-registryState' + (registryInfo.status === 'fresh' ? '' : ' pm-registryStateWarning'), role: 'status' }, registryStatusLabel) : null, body, selectedLocal ? h(LocalDrawer, { plugin: selectedLocal, busy: busy === selectedLocal.name, onClose: function () { setSelectedLocal(null); }, onToggle: function (enabled) { if (selectedLocal)
