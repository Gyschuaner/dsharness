    ...
    window.__ModuleLoader__.load({
        ...
        factory: function (require) {
            ...
            function LocalDrawer(props) {
                ...
                return h('aside', { className: 'pm-drawer', role: 'dialog', 'aria-label': plugin.name + ' 详情' }, h('header', { className: 'pm-drawerHead' }, h('div', { className: 'pm-drawerTitleRow' }, h('h3', { className: 'pm-drawerTitle' }, plugin.name), h('button', { type: 'button', className: 'pm-close', 'aria-label': '关闭详情', onClick: props.onClose }, h(P.IconCloseOutline16))), plugin.repository ? h('a', { className: 'pm-repo', href: 'https://github.com/' + plugin.repository, target: '_blank', rel: 'noreferrer' }, h(Icon, { type: 'github' }), 'github.com/' + plugin.repository, h(Icon, { type: 'external', size: 13 })) : null, h('p', { className: 'pm-drawerDesc' }, plugin.description || '此插件没有提供描述。')), h('div', { className: 'pm-drawerBody' }, h('section', { className: 'pm-section' }, h('h4', { className: 'pm-sectionTitle' }, '版本与来源'), h(KeyValues, { rows: [['版本', plugin.version], ['来源', plugin.source], ['安装声明', plugin.spec]] })), h('section', { className: 'pm-section' }, h('h4', { className: 'pm-sectionTitle' }, '挂载与生效'), h(KeyValues, { rows: [['期望状态', plugin.enabled ? '已启用' : '已停用'], ['当前运行', plugin.runtimePhase || (plugin.runtimeEnabled === false ? '未挂载' : '—')], ['组合行', plugin.rowId], ['变更生效', '重启 Web 后']] })), h(Disclosure, { title: '技术信息' }, h(KeyValues, { rows: [['Host 入口', plugin.manifest.hostEntry], ['Client 入口', plugin.manifest.clientEntry], ['Bundle patch', plugin.manifest.bundlePatch], ['许可证', plugin.license]] }))), h('footer', { className: 'pm-drawerFoot' }, plugin.repository ? h('a', { className: 'pm-btn', href: 'https://github.com/' + plugin.repository, target: '_blank', rel: 'noreferrer' }, '在 GitHub 查看', h(Icon, { type: 'external', size: 13 })) : h(Button, { type: 'button', disabled: true }, '本地来源'), h(Button, { type: 'button', primary: true, disabled: props.busy || plugin.protected, onClick: function () { props.onToggle(!plugin.enabled); } }, plugin.protected ? '运行所必需' : plugin.enabled ? '停用插件' : '启用插件')));
            }
            ...
            function PluginManagerSection(props) {
                ...
                if (loading)
                    ...
                    body = visibleLocal.length === 0 ? h('p', { className: 'pm-empty' }, '没有匹配的本地插件。') : h('div', { className: 'pm-list', 'data-testid': 'local-list' }, visibleLocal.map(function (plugin) {
                        return h('div', { key: plugin.name, className: 'pm-row pm-rowClick' + (selectedLocal && selectedLocal.name === plugin.name ? ' pm-rowSelected' : ''), onClick: function () { setSelectedMarket(null); setSelectedLocal(plugin); } }, h('div', { className: 'pm-rowMain' }, h('span', { className: 'pm-pluginIcon' }, h(Icon, { type: 'plugin', size: 15 })), h('div', { className: 'pm-rowCopy' }, h('div', { className: 'pm-rowTitle' }, plugin.name), h('div', { className: 'pm-rowDesc' }, plugin.description || '此插件没有提供描述。'), h('div', { className: 'pm-rowMeta' }, h('span', null, plugin.version), h('span', null, '·'), h('span', null, plugin.source), plugin.protected ? h('span', null, '· 运行所必需') : null))), h('div', { className: 'pm-rowSide' }, h(Switch, { checked: plugin.enabled, disabled: busy === plugin.name || plugin.protected, label: (plugin.enabled ? '停用 ' : '启用 ') + plugin.name, title: plugin.protected ? '此插件维持当前管理页面运行' : undefined, onChange: function (enabled) { toggle(plugin, enabled); } })));
                    }));
                else
                    body = visibleMarket.length === 0 ? h('p', { className: 'pm-empty' }, '没有匹配的市场插件。') : h('div', { className: 'pm-list', 'data-testid': 'market-list' }, visibleMarket.map(function (item) {
                        var statusLabel = item.status === 'installed' ? '已安装' : item.status === 'update-available' ? '可更新' : '未安装';
                        return h('button', { key: item.id, type: 'button', className: 'pm-row pm-rowClick' + (selectedMarket && selectedMarket.id === item.id ? ' pm-rowSelected' : ''), onClick: function () { openMarket(item); } }, h('div', { className: 'pm-rowCopy' }, h('div', { className: 'pm-rowTitle' }, item.repository), h('div', { className: 'pm-rowDesc' }, item.description)), h('div', { className: 'pm-rowSide' }, h('span', { className: 'pm-status' + (item.status === 'update-available' ? ' pm-statusUpdate' : '') }, statusLabel), h(P.IconChevronRightOutline14)));
                    }));
                return h('section', { className: 'pm-root', 'aria-label': 'Plugin Manager' }, h('header', { className: 'pm-head' }, h('h2', null, 'Plugin'), h('span', { className: 'pm-context' }, 'Web 配置')), h('div', { className: 'pm-tabs', role: 'tablist' }, h('button', { type: 'button', role: 'tab', className: 'pm-tab' + (tab === 'local' ? ' pm-tabOn' : ''), 'aria-selected': tab === 'local', onClick: function () { setTab('local'); setQuery(''); setSelectedMarket(null); } }, '本地插件'), h('button', { type: 'button', role: 'tab', className: 'pm-tab' + (tab === 'market' ? ' pm-tabOn' : ''), 'aria-selected': tab === 'market', onClick: function () { setTab('market'); setQuery(''); setSelectedLocal(null); } }, '插件市场')), h('div', { className: 'pm-toolbar' }, h(Search, { value: query, onChange: setQuery, placeholder: tab === 'local' ? '搜索本地插件' : '搜索插件或 GitHub 仓库' }), tab === 'local' ? h(React.Fragment, null, h(Button, { type: 'button', onClick: function () { setImportOpen(true); } }, h(Icon, { type: 'plus', size: 14 }), '导入插件'), h(Button, { type: 'button', className: 'pm-iconBtn', title: '更多操作', 'aria-label': '更多操作' }, h(Icon, { type: 'dots' }))) : null), tab === 'market' ? h('p', { className: 'pm-helper' }, '精选自 GitHub · 安装前校验 DSH 插件清单') : null, body, selectedLocal ? h(LocalDrawer, { plugin: selectedLocal, busy: busy === selectedLocal.name, onClose: function () { setSelectedLocal(null); }, onToggle: function (enabled) { if (selectedLocal)
