...
const OMITTED_IMAGE_TEXT = '[Image omitted from this model request because the 9-image safety limit was reached.]';
        ...
        for (let imageOrdinal = 0; imageOrdinal < imageCount; imageOrdinal += 1) {
            if (selectedCount >= maxImages) {
                if (selectedOrdinals.size > 0)
                    selectedByMessage.set(messageIndex, selectedOrdinals);
...
export function apply(ctx) {
    ctx.on('llm/stream', (request, next) => {
        ...
        if (limited.request === request)
            return next();
        ctx.logger.warn('trimmed %d of %d image blocks before model dispatch (limit=%d, session=%s)', limited.omittedImages, limited.totalImages, MAX_IMAGES_PER_REQUEST, request.sessionId === undefined ? 'none' : String(request.sessionId));
