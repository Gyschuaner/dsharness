...
| S7 | Client 项目管理页 | 单页项目管理；列表（搜索/已启用/未启用/标签筛选/批量精简/批量启停）；行（名称、必要的项目特化/可更新/来源数徽标、description 第一句、开关）；覆盖式右抽屉（完整描述、当前来源摘要、按需展开全部来源、标签编辑、启用状态、按需技术信息）；旧 Host（apiVersion<6）降级为原 UI | DOM 集成测试：渲染、首句/全文分层、筛选、开关、抽屉、预设预览、事件隔离、布局类不变 |
...
## 8. 2026-08-23 build 19：Skill 页面降噪

- 顶部项目卡压缩为单行上下文条，只保留当前项目、`已启用 / 总数`、切换项目和预设入口；推荐预设与保存预设合并进预设菜单，完整项目路径不再常驻。
- 左侧完整导航由 208px 收窄为 156px，去掉每个分区的重复描述；64px 收起态、正式图标、建设中状态和浏览器本地记忆保持不变。
- 工具栏只常驻搜索、状态筛选、标签筛选和更多操作；批量管理与一键精简移入更多菜单。进入批量模式后才出现复选框和批量提示，单项开关继续隐藏，避免重复启停控件。
- 列表不再显示通用 Skill 图标、`未启用` 徽标和标签；已启用行仍在 catalog 原位使用淡蓝底，开关使用品牌蓝。列表 description 通过标点边界保留第一句话，完整原文始终保留在覆盖式抽屉。
- 抽屉默认只显示启停、完整描述、当前来源摘要与已有标签；全部来源需点击「更改来源」展开，标签输入需点击「添加标签」展开，路径、格式、更新占位和项目特化后续说明统一收到「更多信息」，删除不可用的特化按钮。
- 内置浏览器 1252×945 实测真实 `game` 项目仍为 11/68；156px 导航下列表 x 坐标为 176px、宽 1057.33px，打开 400.67px 抽屉后两者完全不变；来源选项默认不渲染，未来能力文案不出现在主层级。
- 自动化：56 项，52 pass / 0 fail / 4 skip（Windows 权限与符号链接能力限制）；真实 bundle DOM 覆盖 description 首句/全文分层、来源按需展开、低频菜单、稳定顺序、项目隔离与旧 Host 降级。

## 9. 2026-08-23 build 20：单项启停快速路径

- 对应 DP Bug `BUG-F0871ABC`：旧 `setEnabled` 在一次点击中触发 4 次完整 catalog 扫描与 3 次全量 reconcile；真实 `game` 项目包含 68 个身份、79 个来源和约 640 个附属文件，单次启停基线为 1928–2049ms。
- Host 在成功 `catalog` 后保存项目身份快照。单项启停只验证项目来源目录签名、目标来源选择和目标 `SKILL.md` mtime，对目标身份执行一次不扫孤儿的 reconcile，再原子写入项目配置；普通来源、项目原生 Skill 与未修改受管副本均覆盖。缓存缺失、来源树变化、配置损坏/未来版本或来源选择变化时自动退回完整路径。
- 快速路径继续使用文件事务 ledger：目标文件副作用失败时不写真相配置，配置写入失败则还原 stub/frontmatter/受管副本；响应携带 `fastPath`，慢路径额外提供 `fastPathReason` 便于诊断。
- Client 点击即乐观更新当前行、淡蓝底、开关与启用计数，行保持原位；请求期间只显示安静的「保存中」，失败时恢复点击前完整行状态并提示真实错误。
- 自动化：59 项，55 pass / 0 fail / 4 skip；新增普通/项目原生/受管副本快速路径、缓存失效回退、配置写失败回滚和 Client 乐观更新/失败回滚覆盖。
- 重启 3080 后真实 `game` 项目直接 API：普通 `analyze-results` 10 次启停中位 9.6ms、最大 61.7ms；受管副本 `lark-doc` 6 次中位 26.6ms、最大 28.1ms，全部 `fastPath:true`。内置浏览器端到端约 291–304ms（包含约 280ms 浏览器控制轮询），最终恢复 11/68，`analyze-results=false`、`lark-doc=true`。
