...
- **Windows（主开发机，chuansgu，2026-08-28）**：`@deepseek-ai/dsh` 0.1.2-alpha.1
...
`@deepseek-ai/dsh-vision-bridge` 已进入 DSH-005 锁定源码和 base bundle，Plugin Manager 0.2.1
起会将它显示为系统 Bundle 只读条目。0.1.2-alpha.1 的 base bundle 行默认启用；Windows web
profile 固定通过 DP Gateway 的 `Qwen3.8-Flash-Next-FP8` 提供视觉能力。项目插件遵循“默认启用、
只有显式用户配置才关闭”的统一策略，不再由宿主各自补一层默认开关。
...
- **2026-08-28（DSH-034）**：同步官方 `dsh-v0.1.2-alpha.1`，Vision Bridge 迁移到
  ...
  默认启用，Windows web profile 固定使用 `Qwen3.8-Flash-Next-FP8`。本条覆盖 DSH-033 当日
  “base 默认关闭”的临时策略；该旧记录仅用于解释历史配置，不再代表当前启动行为。
...
- **2026-08-24（DSH-008 build 24）**：Skill 页视觉骨架对齐 MCP / Plugin：统一 980px 居中宽度、
  ...
  保留 Skill 独有的项目统计、启停、来源选择、标签和真实市场能力。
...
- **2026-08-24（DSH-027 build 2）**：Plugin 初次读取本地配置和市场索引时显示
  ...
  Plugin 模块从四角向中心接口归位；快速接口也保留约 680ms，避免一帧闪烁。
  `prefers-reduced-motion` 下停止位移和循环，接口失败则退出动画并提供重试。
...
- **2026-08-24（DSH-006）**：把通用“扩展”入口与全页导航壳从
  ...
  `sidebar.footer.action` 并声明根作用域列表 Slot `extension.manager.section`；
  skill-manager 只注册 `skill` 分区，MCP / Plugin 占位归壳所有。Windows web profile
  新增 link 依赖、junction 与独立 Cordis 行，离线组合树和 3080 实机验证通过。
