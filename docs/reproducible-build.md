...
同日新增第十九个补丁。`vision_inspect` 改为紧凑眼睛图标与 `Look ing <elapsed>`
呈现，保留产品设计中的标题间距；视觉层改走 OpenAI 兼容 SSE，并将增量输出作为
`tool/progress` 追加到会话日志。客户端按同一 `rootCallId` 更新已经存在的工具行，
正式 `tool/result` 继承进度后原位完成，因此运行态到完成态不再出现空窗。进度事件
不会进入 `deriveMessages()`，下一轮模型上下文仍只有一对正式 `tool/call` / `tool/result`。
...
2026-08-23 新增第二十四个补丁，完成 DSH-024。视觉桥启用时，原生本地附件服务为
每张新上传图片生成按会话哈希隔离、按附件 SHA 命名且带真实扩展名的只读副本；同一
会话重放保持路径稳定，不向主模型暴露全局对象库路径。图片相邻的持久
`system-reminder` 同时记录附件 ID、名称、媒体类型和本地路径，因此 DeepSeek 可用
现有文件工具做确定性分析，也可继续调用 `vision_inspect` 做语义理解。

视觉子模型提示词和工具结果从固定 JSON 改为简洁、任务导向的 Markdown，删除主模型
无法可靠提供的 `region` / `confidence` 契约。Looking 行在流式阶段跟随最新非空
Markdown 行，完成和刷新后保留完整 Markdown 工具结果。会话视图采用不链接全局对象的
...
2026-08-24 将 DSH-015 旧 RC8 分支的 Code 子工具计时修复迁移到当前 RC2 主线，作为
第二十五个补丁应用。模型不再依赖 `plannedTools` 驱动展示，客户端从增量 `code`
参数中识别直接工具调用，首次识别即记录时间，真实 dispatch 到达后沿用同一计时起点。
迁移后的 7 个聚焦测试文件共 193 条用例通过，源码 tree 锁定为
`56611cffc170612bc5defd0c6040f6beca8a14db`。

2026-08-28 新增第二十六个补丁，完成 DSH-033。视觉桥默认模型、base bundle 中保持
`disabled: true` 的视觉桥行及中英文包文档统一切换到 DP-035 已发布的
`Qwen3.8-Flash-Next-FP8`；旧的 Qwen3.6 CPU 视觉部署记录保留为历史与回退依据。
视觉桥 42 条聚焦测试、翻译配对门禁和 bundle 构建通过，源码 tree 锁定为
`6951f3e071adf9bd6662c3e13687205718469d52`。
